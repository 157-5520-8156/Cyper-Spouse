...
from companion_daemon.world_v2.quick_reaction import QuickReactionPolicy
from companion_daemon.world_v2.quick_reaction_vertical import (
    QuickReactionPolicy as VerticalQuickReactionPolicy,
)


ROOT = Path(__file__).resolve().parents[1]
WATCHDOG = ROOT / "scripts" / "local_appraisal_watchdog.sh"
    ...
    environment.update(
        {
            ...
            "TMPDIR": str(tmp_path),
            "LOCAL_APPRAISAL_WATCHDOG_LOG_FILE": str(tmp_path / "watchdog.log"),
            ...
            "LOCAL_APPRAISAL_CAPACITY_COOLDOWN_SECONDS": "0",
...
def test_local_mlx_server_disables_unsafe_multi_prompt_batching() -> None:
    command = (ROOT / "scripts" / "run_local_appraisal.sh").read_text()
...
def test_local_appraisal_watchdog_probes_generation_not_only_the_listener() -> None:
...
def test_healthy_model_list_treats_one_inference_timeout_as_busy_not_wedged(
    ...
    (tmp_path / "girl-agent-local-appraisal-watchdog.last").write_text(str(int(time.time())))

    result = _run_watchdog(environment)

    assert result.returncode == 0
    assert "/v1/models" in (tmp_path / "curl.log").read_text()
    assert (tmp_path / "girl-agent-local-appraisal-watchdog.failures").read_text().strip() == "1"
...
def test_known_daemon_capacity_busy_skips_generation_probe_and_restart(
    ...
    capacity = tmp_path / "girl-agent-local-appraisal.capacity"
    capacity.mkdir()
    (capacity / "state").write_text(
        f"{time.time() + 300:.6f}\ndaemon:test:1\nactive\n"
    )
    (tmp_path / "girl-agent-local-appraisal-watchdog.failures").write_text("2")
    ...
    assert not (tmp_path / "launchctl.log").exists()
    assert not (tmp_path / "girl-agent-local-appraisal-watchdog.failures").exists()
...
def test_watchdog_timeout_keeps_a_busy_lease_before_any_new_probe(
    ...
    environment["LOCAL_APPRAISAL_CAPACITY_COOLDOWN_SECONDS"] = "300"
    ...
    state = (
        tmp_path / "girl-agent-local-appraisal.capacity" / "state"
    ).read_text()
    assert "watchdog_cooldown" in state
    assert (
        tmp_path / "girl-agent-local-appraisal-watchdog.failures"
    ).read_text().strip() == "1"
    assert not (tmp_path / "launchctl.log").exists()
...
def test_three_consecutive_inference_failures_restart_a_healthy_listener(
    ...
    assert first.returncode == second.returncode == 0
    assert (tmp_path / "girl-agent-local-appraisal-watchdog.failures").read_text().strip() == "2"
    assert not (tmp_path / "launchctl.log").exists()
...
def test_both_quick_reaction_implementations_share_the_inline_budget() -> None:
    assert QuickReactionPolicy().gate_timeout_seconds == 0.25
    assert VerticalQuickReactionPolicy().gate_timeout_seconds == 0.25
