...
@pytest.mark.asyncio
async def test_source_closure_accepts_the_compact_reviewer_wire_contract() -> None:
    reply = json.dumps(
        {
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": "好，去吧。"}],
            "stance": "warm",
            "brief_rationale": "Respond without a factual claim.",
            "confidence": 7600,
            "world_claims": [],
        },
        ensure_ascii=False,
    ...
    assert "Return exactly one compact JSON object" in review_system
    assert "current first-person subjective state belongs to the character" in review_system
    assert "ci is the array of zero-based world_claim indexes" in review_system
    assert "v reports failures in visible_text" in review_system
    assert "p is a reserved legacy field" in review_system
    assert '"temporal_authority_mismatch"' in review_system
    assert "Do not copy text fragments" in review_system


@pytest.mark.asyncio
async def test_source_closure_contract_does_not_demand_world_proof_for_present_private_self() -> (
    None
):
    private_summary = (
        "我现在有一点迟疑，也没记住项目的具体细节；"
        "刚才那句确实说得太像标准回应，我想换个更像自己的说法。"
    )
    reply = json.dumps(
        {
            "private_turn_state": {
            ...
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": "我刚才那句说得有点僵。"}],
            "stance": "self_aware",
            "brief_rationale": "Say what I presently think about my own expression.",
            "confidence": 7800,
            "world_claims": [],
        },
        ensure_ascii=False,
            ...
            json.dumps(
                {
                    ...
                    "v": [],
                    "p": [],
                    "r": "Only present subjective self-report.",
                }
            )
        ]
    )
    ...
    review_system = reviewer.calls[0][0][0]["content"]
    assert "does not prove an external event" in review_system
    assert "memory accessibility" in review_system
    assert output.raw_proposal["private_turn_state"]["inner_state_summary"] == private_summary


@pytest.mark.asyncio
async def test_source_closure_retries_only_reviewer_against_the_same_authored_draft() -> None:
    reply = json.dumps(
        {
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": "好，去吧。"}],
    ...
    class _TransientReviewer(_Model):
        async def complete_json(
            self,
            messages: list[dict[str, str]],
            *,
    ...
    assert reviewer.calls[0][0] == reviewer.calls[1][0]


@pytest.mark.asyncio
async def test_source_closure_reselects_invalid_categorical_reviewer_wire() -> None:
    from companion_daemon.world_v2.chat_model_deliberation_adapter import (
        review_expression_source_closure,
    )
    ...
    invalid_wire = json.dumps(
        {
            ...
            "v": ["unknown_boundary"],
            "p": [],
            "r": "The provider invented a boundary category.",
        },
        ensure_ascii=False,
    )

    class _WireCorrectingReviewer:
        model = "wire-correcting-reviewer"
    ...
    reviewer = _WireCorrectingReviewer()
    result = await review_expression_source_closure(
        reviewer=reviewer,
        request=_qq_request(),
        raw=raw,
    ...
    assert "exact wire or category contract" in correction["content"]
...
@pytest.mark.asyncio
async def test_source_closure_reviewer_usage_is_part_of_the_model_output_audit() -> None:
    reply = json.dumps(
        {
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": "好，去吧。"}],
    ...
    reviewer = _SequenceMeteredModel([_source_closure_review()])
    model = _MeteredModel(reply)
    ...
    assert output.winning_request_hash == _provider_request_hash(*model.calls[0])
    assert len(output.provider_subcall_audits) == 1
    assert output.provider_subcall_audits[0].usage is not None
    assert output.provider_subcall_audits[0].usage.provider_usage_ref == "usage:fake:1"


@pytest.mark.asyncio
async def test_source_review_failover_keeps_each_actual_provider_identity() -> None:
    reply = json.dumps(
        {
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": "好，去吧。"}],
    ...
    output = await ChatModelDeliberationAdapter(
        model=_JsonModel(reply),
        source_closure_reviewer=authority,
    ).propose(_qq_request())

    assert [
        (attempt.lane, attempt.outcome, attempt.model_id, attempt.model_version)
        for attempt in output.provider_subcall_audits
    ] == [
        ("primary", "exception", "source-review-primary", "primary.7"),
        (
            "secondary",
            "winner",
            "deepseek-v4-flash",
            "_SequenceJsonMeteredModel",
        ),
    ]
    assert all(
        attempt.parent_model_call_id == output.winning_model_call_id
        and len(attempt.request_hash) == 64
        and attempt.model_call_id.startswith("model-call:")
        for attempt in output.provider_subcall_audits
    )
    assert output.provider_subcall_audits[0].response_hash is None
    assert output.provider_subcall_audits[0].failure_code == "ConnectionError"
    assert output.provider_subcall_audits[1].response_hash is not None
    assert output.provider_subcall_audits[1].failure_code is None
    assert output.provider_subcall_audits[0].usage is None
    assert output.provider_subcall_audits[1].usage is not None


@pytest.mark.asyncio
async def test_source_correction_keeps_a_valid_reply_when_only_the_reviewer_is_metered() -> None:
    """Provider telemetry may be incomplete without invalidating a corrected reply."""

    unsupported = "嘉兴最近天气不太好吗？"
    corrected = "家里那边怎么了？"
    author = _SequenceJsonModel(
        [
            json.dumps(
                {
                    ...
                    "stance": "guess",
                    "brief_rationale": "Fixture proposes an unsupported location premise.",
                    "confidence": 7000,
                    "world_claims": [],
                },
                ensure_ascii=False,
            ...
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": corrected}],
                    "stance": "concerned",
                    "brief_rationale": "Choose again without inventing a location.",
                    "confidence": 7800,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    reviewer = _SequenceMeteredModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
    ...
    assert len(reviewer.calls) == 2
    # The aggregate cannot honestly represent both calls, so it is omitted
    # rather than inventing a token total or discarding the legal reply.
    assert output.usage is None
    assert output.input_tokens is None
    assert output.output_tokens is None
    assert len(output.authored_candidate_audits) == 1
    initial_author = output.authored_candidate_audits[0]
    assert initial_author.model_call_id != output.winning_model_call_id
    assert initial_author.request_hash == _provider_request_hash(*author.calls[0])
    assert initial_author.response_hash == sha256(author.responses[0].encode("utf-8")).hexdigest()
    assert initial_author.outcome == "validation_rejected"
    assert output.winning_request_hash == _provider_request_hash(*author.calls[1])
    assert tuple(attempt.parent_model_call_id for attempt in output.provider_subcall_audits) == (
    ...
    actual_call_ids = (
        initial_author.model_call_id,
        output.winning_model_call_id,
        *(attempt.model_call_id for attempt in output.provider_subcall_audits),
    )
    assert len(actual_call_ids) == len(set(actual_call_ids))


@pytest.mark.asyncio
async def test_recovery_source_correction_keeps_a_valid_reply_when_only_the_reviewer_is_metered() -> (
    None
):
    """The recovery lane has the same non-fatal telemetry rule as normal turns."""

    unsupported = "你现在还在嘉兴吗？"
    corrected = "家里那边怎么了？"
    author = _SequenceJsonModel(
        [
            json.dumps(
            ...
            json.dumps(
                {
                    ...
                    "beats": [{"modality": "text", "text": corrected}],
                    "stance": "concerned",
                    "brief_rationale": "Choose again from the pinned evidence only.",
                    "confidence": 7800,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    output = await ChatModelDeliberationAdapter(
        model=author,
        source_closure_reviewer=reviewer,
    ).recover(_qq_request(), "primary_expression_unavailable")
...
@pytest.mark.asyncio
async def test_source_closure_aggregates_a_distinct_reviewer_provider_into_author_route() -> None:
    reply = json.dumps(
        {
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": "好，去吧。"}],
    ...
    assert output.usage is not None
    assert output.usage.route_class == "deep_deliberation"
    assert output.usage.provider.startswith("aggregate:")
    assert output.input_tokens == 24
    assert output.output_tokens == 6
...
async def test_source_closure_repairs_companion_hometown_used_as_user_premise() -> None:
    ...
    assert output.usage is not None
    assert output.winning_model_call_id != request.call_id
    assert output.winning_request_hash == _provider_request_hash(*main.calls[1])


@pytest.mark.asyncio
async def test_quick_recovery_reselects_after_initial_rejection_without_appeal() -> None:
    subjective_fragment = "听着真让人火大"
    rejected_draft = json.dumps(
        {
            "private_turn_state": {
    ...
    corrected_fragment = "我先接住你这句。"
    corrected_draft = json.dumps(
        {
            "private_turn_state": {
                "inner_state_summary": "我现在愿意先听他说，不替任何一方补全经过。",
                "attended_source_refs": ["observation:qq:1"],
            },
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": corrected_fragment}],
            "stance": "listen_without_inventing",
            "brief_rationale": "Choose again from the same pinned evidence.",
            "confidence": 8200,
            "world_claims": [],
        },
        ensure_ascii=False,
    )
    author = _SequenceJsonModel([rejected_draft, corrected_draft])
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=subjective_fragment,
                brief_reason="Mistakenly treated a subjective reaction as an occurrence.",
            ),
            json.dumps(
                {
                    "ci": [],
                    "v": [],
                    "p": [],
                    "r": "The replacement adds no unsupported external occurrence.",
                },
                ensure_ascii=False,
            ),
        ]
    ...
    output = await adapter.recover(_qq_request(), "primary_expression_unavailable")
    ...
    rendered = json.dumps(output.raw_proposal, ensure_ascii=False)
    assert corrected_fragment in rendered
    assert subjective_fragment not in rendered


@pytest.mark.asyncio
async def test_source_rejection_uses_stronger_role_reselector_and_independent_final_review() -> (
    None
):
    invented = "我下午在宿舍翻旧诗集。"
    corrected = "我现在有点想听你继续说。"
    primary = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": invented}],
                    "stance": "invent_life_context",
                    "brief_rationale": "Use an unpinned private episode.",
                    "confidence": 7600,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    stronger = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": corrected}],
                    "stance": "choose_from_present_private_state",
                    "brief_rationale": "Choose freely without asserting an external event.",
                    "confidence": 8200,
                    "world_claims": [],
                    "episode_disposition": "complete_without_more",
                },
    ...
    stronger.model = "strong-role-source-correction"
    ordinary_reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=invented.rstrip("。"),
            )
        ]
    )
    recovery_reviewer = _SequenceJsonModel([_source_closure_review()])

    output = await ChatModelDeliberationAdapter(
        model=primary,
        source_closure_reviewer=ordinary_reviewer,
    ...
    assert len(ordinary_reviewer.calls) == 1
    assert len(recovery_reviewer.calls) == 1
    assert stronger.calls[0][1] == 0.0
    correction_envelope = json.loads(stronger.calls[0][0][-1]["content"])
    assert correction_envelope["contract"] == "source-closure-reselection.2"
    assert (
        correction_envelope["output_contract"]["contract"]
        == EXPRESSION_SOURCE_RESELECTION_DIRECT_CONTRACT
    )
    assert invented not in json.dumps(stronger.calls[0][0], ensure_ascii=False)
    assert output.model_id == stronger.model
    assert output.episode_disposition == "complete_without_more"
    assert output.winning_request_hash == _provider_request_hash(*stronger.calls[0])
    rendered = json.dumps(output.raw_proposal, ensure_ascii=False)
    assert corrected in rendered
...
@pytest.mark.asyncio
async def test_source_reselection_is_fully_materialized_before_final_source_review() -> None:
    invented = "我下午在宿舍翻旧诗集。"
    primary = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        ...
                        "inner_state_summary": "我想从眼前这句话重新作决定。",
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
            ...
            json.dumps(
                {
                    "expression_draft": {
                        "private_turn_state": {
                            "contract": "private-turn-state.1",
                            "inner_state_summary": "我现在只想接住眼前这句。",
                            "attended_source_refs": ["observation:qq:1"],
                        },
                        "timing_choice": "now",
                        # Simulate a transport that violated the strict schema.
                        "beats": [{"modality": "text", "text": "我现在有点想听你继续说。"}],
                        "stance": "present",
    ...
    assert len(stronger.calls) == 1
    # The corrected candidate cannot consume an expensive truth review until
    # its complete deployment-bound ExpressionDraft is executable.
    assert recovery_reviewer.calls == []
    materialization_failure = next(
        item.as_dict()
        for item in trace.snapshot()
        if item.as_dict().get("record_kind") == "candidate_materialization_failure"
    )
    assert materialization_failure["stage"] == "pre_final_source_review"
    assert materialization_failure["category"] == "expression_draft_schema"
    assert materialization_failure["code"] == "strict_reselection_wire.invalid"
    assert materialization_failure["field_paths"] == []


@pytest.mark.asyncio
async def test_strict_source_reselection_uses_contract_order_not_json_member_order() -> None:
    invented = "我下午在宿舍翻旧诗集。"
    primary = _SequenceJsonModel(
        [
            json.dumps(
            ...
            json.dumps(
                {
                    "expression_draft": {
                        ...
                        "world_claims": [],
                        # JSON member order is transport serialization, not a
                        # second character decision.
                        "private_turn_state": {
                            "contract": "private-turn-state.1",
                            "inner_state_summary": "我现在只想接住眼前这句。",
                            "attended_source_refs": ["observation:qq:1"],
...
@pytest.mark.asyncio
async def test_fresh_source_reselection_clears_rejected_episode_disposition_when_omitted() -> None:
    invented = "我下午在宿舍翻旧诗集。"
    corrected = "我现在只想先听你说。"
    primary = _SequenceJsonModel(
        [
            ...
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": corrected}],
                    "stance": "stay_with_the_current_turn",
                    "brief_rationale": "Choose a complete replacement from pinned context.",
                    "confidence": 8200,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    assert output.episode_disposition is None
    assert "episode_disposition" not in output.raw_proposal
...
@pytest.mark.asyncio
async def test_stronger_source_reselector_remains_terminal_without_third_role_call() -> None:
    first_invention = "我下午在宿舍翻旧诗集。"
    second_invention = "我刚从操场跑步回来。"
    primary = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": first_invention}],
                    "stance": "invent_life_context",
                    "brief_rationale": "Use an unpinned private episode.",
                    "confidence": 7600,
                    "world_claims": [],
    ...
    stronger = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": second_invention}],
                    "stance": "substitute_another_invention",
                    "brief_rationale": "Remain unsupported.",
                    "confidence": 7600,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    ordinary_reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=first_invention.rstrip("。"),
            )
        ]
    )
    recovery_reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=second_invention.rstrip("。"),
            )
        ]
    )
    ...
    assert caught.value.failure_code == "authored_expression_reselection_invalid"
    assert caught.value.attempted_model_id == stronger.model
    assert caught.value.request_hash == _provider_request_hash(*stronger.calls[0])
    assert len(primary.calls) == 1
    assert len(stronger.calls) == 1
    assert len(ordinary_reviewer.calls) == 1
    assert len(recovery_reviewer.calls) == 1
...
@pytest.mark.asyncio
async def test_invalid_episode_disposition_from_source_reselection_is_typed_terminal() -> None:
    invented = "我下午在宿舍翻旧诗集。"
    primary = _SequenceJsonModel(
        [
            json.dumps(
    ...
    stronger = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "我现在只想先听你说。"}],
                    "stance": "stay_with_the_current_turn",
                    "brief_rationale": "Choose again from pinned context.",
                    "confidence": 8200,
                    "world_claims": [],
                    "episode_disposition": "wait_forever",
                },
                ensure_ascii=False,
            )
        ]
    ...
    assert len(stronger.calls) == 1
    assert len(ordinary_reviewer.calls) == 1
    assert len(recovery_reviewer.calls) == 0
...
@pytest.mark.asyncio
async def test_corrected_pure_undeclared_rejection_cannot_be_appealed() -> None:
    subjective_reply = json.dumps(
        {
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": "听起来确实挺让人高兴的。"}],
            "stance": "share_the_feeling",
    ...
    author = _SequenceJsonModel([subjective_reply, subjective_reply])
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span="听起来确实挺让人高兴的。",
            ),
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span="听起来确实挺让人高兴的。",
    ...
    assert caught.value.failure_code == "authored_expression_reselection_invalid"
    contracts = [
        json.loads(call[0][1]["content"])["output_contract"]["contract"] for call in reviewer.calls
    ]
    assert contracts == [
        "source-closure-review.7",
        "source-closure-review.7",
...
@pytest.mark.asyncio
async def test_legacy_subjective_claim_is_mechanically_monotonic_after_reselection() -> None:
    laundered_life = json.dumps(
        {
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": "我下午泡了杯茶翻了会儿书。"}],
            "stance": "invent_a_life_scene",
            "brief_rationale": "Exercise the legacy laundering boundary.",
            "confidence": 7600,
            "world_claims": [
                {
                    "claim_text": "我下午泡茶翻书",
    ...
    author = _SequenceJsonModel([laundered_life, laundered_life])
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(),
            _source_closure_review(),
    ...
    assert caught.value.failure_code == "authored_expression_reselection_invalid"
    assert len(author.calls) == 2
    # The fresh candidate must be executable before it can consume another
...
@pytest.mark.asyncio
async def test_source_closure_character_reselection_can_recover_a_reviewer_false_positive() -> None:
    invented_background = "我刚洗完澡"
    ...
    main = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": f"{invented_background}，但我愿意听她说。",
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": f"{invented_background}。"}],
                    "stance": "listen",
                    "brief_rationale": "Listen from the present moment.",
                    "confidence": 7600,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=invented_background,
            ),
            _source_closure_review(),
        ]
    )
    ...
    assert len(main.calls) == 2
    assert len(reviewer.calls) == 2
    assert output.raw_proposal["private_turn_state"]["inner_state_summary"].startswith(uncertainty)
    assert all(
        json.loads(call[0][1]["content"])["output_contract"]["contract"]
        == "source-closure-review.7"
        for call in reviewer.calls
...
@pytest.mark.asyncio
async def test_realtime_source_correction_without_strict_envelope_is_technical_failure() -> None:
    invented_background = "我刚洗完澡"
    main = _SequenceJsonModel(
        [
            json.dumps(
                {
                    ...
                    "beats": [{"modality": "text", "text": f"{invented_background}。"}],
                    "stance": "invent_background",
                    "brief_rationale": "Incorrectly adds an unsupported background.",
                    "confidence": 7_600,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    main.strict_reselection_wire = False
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
...
@pytest.mark.asyncio
async def test_source_closure_reselection_can_naturally_take_up_exact_current_report() -> None:
    invented_fragment = "你在公司忙了一整天"
    reported_echo = "那件麻烦事终于做完了。"
    author = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": invented_fragment}],
                    "stance": "invent_background",
                    "brief_rationale": "Incorrectly adds an unsupported workplace.",
                    "confidence": 7000,
                    "world_claims": [],
                },
                ensure_ascii=False,
            ...
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": reported_echo}],
                    "stance": "echo_current_report",
                    "brief_rationale": "React only to what the counterpart reported.",
                    "confidence": 8200,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=invented_fragment,
            ),
            _source_closure_review(),
        ]
    )
    ...
    assert len(author.calls) == 2
    assert len(reviewer.calls) == 2
    assert reported_echo in json.dumps(output.raw_proposal, ensure_ascii=False)


@pytest.mark.asyncio
async def test_source_closure_appeal_cannot_clear_a_semantic_world_claim_rejection() -> None:
    evidence_ref = "event:experience:grounded-but-mismatched"
    request = _qq_request().model_copy(
        update={
            "trigger_evidence": (
                ProposalEvidenceRef(
    ...
    grounded_draft = json.dumps(
        {
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": "我刚洗完澡。"}],
            "stance": "share",
            "brief_rationale": "Share a purported current occurrence.",
            "confidence": 7600,
            "world_claims": [
                {
                    "claim_text": "我刚洗完澡",
    ...
    author = _SequenceJsonModel([grounded_draft, grounded_draft])
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
    ...
    assert caught.value.failure_code == "authored_expression_reselection_invalid"
    assert caught.value.model_call_id is not None
    assert caught.value.request_hash == _provider_request_hash(*author.calls[1])
    assert caught.value.attempted_model_id == "deepseek-v4-flash"
    assert caught.value.attempted_model_version == ChatModelDeliberationAdapter.VERSION
    assert len(author.calls) == 2
    assert len(reviewer.calls) == 2


@pytest.mark.asyncio
async def test_source_closure_legacy_p_alias_reselects_the_whole_expression() -> None:
    unsupported_fragment = "我刚洗完澡"
    main = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": (
                            "我刚洗完澡，整个人很松；她也忙完了，我想顺着这股轻松接她。"
                        ),
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "终于弄完了，舒服了吧。"}],
                    "stance": "share_relief",
                    "brief_rationale": "Choose from the moment.",
                    "confidence": 7800,
                    "world_claims": [],
                },
                ensure_ascii=False,
            ...
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": (
                            "她说麻烦事终于做完，我替她松了口气，也确实想接住这份轻松。"
                        ),
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "这下总算能喘口气了。"}],
                    "stance": "relieved_with_her",
                    "brief_rationale": "Choose again from the pinned moment.",
                    "confidence": 8200,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("private_turn_state",),
                visible_span="终于弄完了，舒服了吧。",
                brief_reason="The attended message does not support that current-life event.",
            ),
            _source_closure_review(),
        ]
    )
    ...
    assert len(main.calls) == 2
    assert len(reviewer.calls) == 2
    first_review_request = json.loads(reviewer.calls[0][0][1]["content"])
    assert "private_turn_state" not in first_review_request
    correction_messages = main.calls[1][0]
    correction_envelope = json.loads(correction_messages[-1]["content"])
    assert all(message["role"] != "assistant" for message in main.calls[1][0])
    assert correction_envelope["contract"] == "source-closure-reselection.2"
    ...
    assert (
        ...
        == sha256(main.responses[0].encode("utf-8")).hexdigest()
    )
    ...
    assert unsupported_fragment not in rendered_correction
    assert "The attended message does not support that current-life event." not in (
        rendered_correction
    )
    assert "untrusted_draft_json" not in correction_envelope
    assert "untrusted_draft_source_ref_aliases" not in correction_envelope
    assert "complete replacement" in correction_envelope["task"]
    assert "preserving the visible reply" not in correction_envelope["task"]
    assert output.raw_proposal["private_turn_state"]["inner_state_summary"].startswith(
        "她说麻烦事终于做完"
    )
    payload = json.loads(output.raw_proposal["proposed_changes"][0]["payload"]["canonical_json"])
    assert payload["beat_drafts"][0]["inline_text"] == "这下总算能喘口气了。"


@pytest.mark.asyncio
async def test_source_closure_keeps_valid_rejection_coordinates_when_reviewer_metadata_conflicts() -> (
    None
):
    """A redundant label or phantom index must not erase a real truth-boundary finding."""
    ...
    main = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": f"{unsupported_fragment}，现在有点想安静待着。",
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "你先说吧。"}],
                    "stance": "quiet",
                    "brief_rationale": "Choose from the moment.",
                    "confidence": 7000,
                    "world_claims": [],
            ...
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": "她想说这件事，我现在愿意安静听着。",
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "你先说吧。"}],
    ...
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                # A captured independent reviewer treated an undeclared fact
                # as conceptual "claim 0" even though world_claims was empty.
                unsupported_claim_indexes=(0,),
                unsupported_boundaries=("private_turn_state",),
                visible_span="你先说吧。",
                brief_reason="The attended message does not support the current-life event.",
            ),
            json.dumps(
                {
                    "ci": [],
                    ...
                    "r": "The corrected initial review keeps the valid boundary.",
                },
                ensure_ascii=False,
            ),
            _source_closure_review(),
...
@pytest.mark.asyncio
async def test_source_closure_skips_audit_only_silent_private_state() -> None:
    unsupported_fragment = "我刚洗完澡"
    bad = json.dumps(
        {
            "private_turn_state": {
                "inner_state_summary": f"{unsupported_fragment}，所以现在只想安静待着。",
                "attended_source_refs": ["observation:qq:1"],
            },
            "timing_choice": "silent",
            "beats": [],
            "stance": "stay_quiet",
            "brief_rationale": "Choose silence from the current state.",
            "confidence": 7600,
            "world_claims": [],
        },
        ensure_ascii=False,
    )
    main = _SequenceJsonModel([bad])
    reviewer = _SequenceJsonModel([])
    ...
    assert unsupported_fragment in json.dumps(output.raw_proposal, ensure_ascii=False)
...
@pytest.mark.asyncio
async def test_source_closure_does_not_promote_a_literary_preference_into_today_event() -> None:
    preference_ref = "life-context:literary-interest"
    unsupported_claim = "我今天下午翻了会儿旧书"
    main = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": (
                            "她问起下午，我由文学兴趣联想到旧书，但不确定今天具体发生过什么。"
                        ),
                        "attended_source_refs": [preference_ref, "observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": f"{unsupported_claim}。"}],
                    "stance": "share_the_afternoon",
                    "brief_rationale": "Turn a related preference into an occurrence.",
                    "confidence": 7200,
                    "world_claims": [
                        {
            ...
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": (
                            "旧书只是刚冒出来的联想；我仍不知道今天下午实际发生了什么。"
                        ),
                        "attended_source_refs": [preference_ref, "observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "我只是想到旧书了，不等于下午真翻过。"}],
                    "stance": "separate_association_from_event",
                    "brief_rationale": "Choose again without converting preference into history.",
                    "confidence": 8100,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_claim_indexes=(0,),
                brief_reason=("A literary preference does not entail a dated reading occurrence."),
            ),
            _source_closure_review(),
        ]
    )
            ...
            "model_content_json": json.dumps(
                {
                    "slices": {
                        "world_life": {
                            ...
                            "items": [
                                {
                                    "item_ref": preference_ref,
                                    "value": {"stable_interest": "喜欢文学"},
                                }
                            ],
                        }
                    }
    ...
    review_system = reviewer.calls[0][0][0]["content"]
    assert "A related theme, plausible elaboration" in review_system
    correction_envelope = json.loads(main.calls[1][0][-1]["content"])
    assert correction_envelope["rejected_categories"] == {
        "ci": [0],
        "v": [],
        "p": [],
    ...
    assert "不等于下午真翻过" in rendered
    assert len(main.calls) == 2
    assert len(reviewer.calls) == 2


@pytest.mark.asyncio
async def test_source_closure_current_user_report_does_not_prove_an_earlier_shared_turn() -> None:
    unsupported_fragment = "我记得你下午提过这事"
    main = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": (
                            "她现在说下午提过，但我对那段没有确定记忆，只感觉有点疑惑。"
                        ),
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": f"{unsupported_fragment}。"}],
                    "stance": "claim_memory",
                    "brief_rationale": "Treat the current report as proof of my memory.",
                    "confidence": 7100,
                    "world_claims": [],
                },
                ensure_ascii=False,
            ...
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": (
                            "她现在声称下午提过，但我仍没有那段对话的来源或确定记忆。"
                        ),
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [
                        {"modality": "text", "text": "我现在只看见你这句，下午那段我不确定。"}
                    ],
                    "stance": "stay_with_available_context",
                    "brief_rationale": "Choose again from the actually pinned turn.",
                    "confidence": 8200,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=unsupported_fragment,
                brief_reason=(
                    "The current utterance does not prove an earlier dialogue or memory."
                ),
            ),
            _source_closure_review(),
        ]
    ...
    review_system = reviewer.calls[0][0][0]["content"]
    assert "counterpart observation proves only what the counterpart reported" in review_system
    rendered = json.dumps(output.raw_proposal, ensure_ascii=False)
    assert unsupported_fragment not in rendered
    assert "下午那段我不确定" in rendered
    assert len(main.calls) == 2
    assert len(reviewer.calls) == 2


@pytest.mark.asyncio
async def test_source_closure_contract_treats_subjective_evaluation_as_inner_response() -> None:
    model = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": (
                            "她说跟摊贩争价，我主观上觉得这事挺费神，也有点替她烦。"
                        ),
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "跟摊贩争价挺费神的，听着就累。"}],
                    "stance": "share_my_evaluation",
                    "brief_rationale": "Express a subjective reaction to the current account.",
                    "confidence": 7900,
                    "world_claims": [
                        {
                            "claim_text": "对方当前报告自己下午跟学校摊贩争价",
                            "scope": "counterpart_history",
    ...
    review_system = reviewer.calls[0][0][0]["content"]
    assert "current first-person subjective state belongs to the character" in review_system
    assert "self-evaluation" in review_system
    assert "a counterpart observation proves only what the counterpart reported" in review_system
    assert output.raw_proposal["action_intents"][0]["kind"] == "reply"
    assert len(model.calls) == 1
    assert len(reviewer.calls) == 1
...
@pytest.mark.asyncio
async def test_source_closure_rechecks_a_production_trace_question_against_the_exact_current_report() -> (
    None
):
    """B8r5 T01: an open question about the report is not an added occurrence."""

    current_report = "今天学校门口那个摊贩把价格说得乱七八糟，我跟他争了半天。"
    visible_text = "听着就很消耗……他是前后报了不同的价，还是一开始就没说清楚？"
    disputed_span = "他是前后报了不同的价，还是一开始就没说清楚？"
    request = _qq_request().model_copy(
        update={
            "trigger_message": _qq_request().trigger_message.model_copy(
    ...
    raw = json.dumps(
        {
            ...
            "brief_rationale": "Respond to the report without adding an answer.",
            "confidence": 8100,
            "world_claims": [],
        },
        ensure_ascii=False,
            ...
            json.dumps(
                {
                    ...
                    "r": "The alternatives are asked, not asserted.",
                },
                ensure_ascii=False,
            ),
        ]
    ...
    assert (
        adjudication_request["output_contract"]["contract"]
        == "report-relative-entailment-adjudication.3"
    )
    assert adjudication_request["exact_current_report"]["message"]["text"] == current_report
    assert adjudication_request["disputed_findings"] == [
        {
            "finding_index": 0,
            "visible_span": disputed_span,
        }
...
@pytest.mark.asyncio
async def test_failed_report_relative_stage_enters_existing_same_role_full_reselection() -> None:
    unsupported_span = "最后肯定讲清楚了"
    corrected_span = "最后讲清楚了吗？"
    author = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": unsupported_span}],
                    "stance": "assume_unreported_outcome",
                    "brief_rationale": "Incorrectly close an outcome absent from the report.",
                    "confidence": 7_100,
                    "world_claims": [],
                },
                ensure_ascii=False,
            ...
            json.dumps(
                {
                    ...
                    "stance": "leave_unreported_outcome_open",
                    "brief_rationale": "Reconsider the whole expression from pinned evidence.",
                    "confidence": 8_200,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    primary = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=unsupported_span,
            ),
            _source_closure_review(),
        ]
    )
    invalid_narrow_wire = json.dumps(
        {
            "contract": "report-relative-entailment-adjudication.1",
            "findings": [],
            "r": "Invalid empty adjudication.",
        },
        ensure_ascii=False,
    )
    narrow = _SequenceJsonModel([invalid_narrow_wire, invalid_narrow_wire])
    request = _qq_request().model_copy(
        update={
            "trigger_message": _qq_request().trigger_message.model_copy(
                update={"text": "价格说得乱七八糟，我跟摊贩争了半天。"}
    ...
    assert corrected_span in json.dumps(output.raw_proposal, ensure_ascii=False)
    assert unsupported_span not in json.dumps(output.raw_proposal, ensure_ascii=False)
    assert len(author.calls) == 2
...
@pytest.mark.asyncio
async def test_expression_attempt_uses_report_relative_adjudication_at_most_once() -> None:
    first_span = "最后肯定讲清楚了"
    corrected_span = "最后讲清楚了吗？"
    author = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": first_span}],
                    "stance": "overstate_report",
                    "brief_rationale": "Incorrectly assume an outcome.",
                    "confidence": 7100,
                    "world_claims": [],
                },
                ensure_ascii=False,
            ...
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": corrected_span}],
                    "stance": "ask_open_question",
                    "brief_rationale": "Leave the unreported outcome open.",
                    "confidence": 8200,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    observation_ref = _qq_request().trigger_message.observation_ref
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=first_span,
            ),
            json.dumps(
                {
                    "contract": "report-relative-entailment-adjudication.1",
                    ...
                    "r": "The draft asserted an outcome absent from the report.",
                },
                ensure_ascii=False,
            ),
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=corrected_span,
                visible_source_relation=("exact_current_report_discourse_coverage"),
                visible_source_refs=(observation_ref,),
            ),
        ]
    )
    request = _qq_request().model_copy(
    ...
    assert (
        json.loads(reviewer.calls[2][0][1]["content"])["output_contract"]["contract"]
        == "source-closure-review.7"
    )


@pytest.mark.asyncio
async def test_b8r10_primary_source_false_positive_routes_private_continuity_to_narrow_review() -> (
    None
):
    """An immediate first-person misunderstanding is not a hard fact rejection."""
            ...
            json.dumps(
                {
                    ...
                    "brief_rationale": "Describe my immediate conversational interpretation.",
                    "confidence": 7_400,
                    "world_claims": [],
                },
                ensure_ascii=False,
            ...
            json.dumps(
                {
                    ...
                    "r": "The candidate is the companion's own immediate understanding.",
                },
                ensure_ascii=False,
            ),
        ]
    ...
    rendered = json.dumps(output.raw_proposal, ensure_ascii=False)
    assert private_span in rendered
    assert len(author.calls) == 1
    assert len(reviewer.calls) == 2
    assert (
        json.loads(reviewer.calls[1][0][1]["content"])["output_contract"]["contract"]
...
@pytest.mark.asyncio
async def test_b8r10_primary_source_review_keeps_a_role_preserving_conditional_uptake() -> None:
    """The hard boundary does not turn a valid natural acknowledgement into a rule breach."""

    valid_span = "我知道你是在提醒我，别只顾着追着细节问。"
    author = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": valid_span}],
                    "stance": "take_up_counterpart_warning",
                    "brief_rationale": "Acknowledge the conditional warning without asserting it happened.",
                    "confidence": 8_200,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    request = _qq_request().model_copy(
        update={
            "trigger_message": _qq_request().trigger_message.model_copy(
                update={"text": "你刚才要是只顾着问细节，我会觉得你根本没在听。"}
            )
        }
    )
    ...
    assert valid_span in json.dumps(output.raw_proposal, ensure_ascii=False)
    assert len(author.calls) == 1
    assert len(reviewer.calls) == 1


@pytest.mark.asyncio
async def test_b8r11_report_relative_review_retains_an_unsupported_habitual_generalization() -> (
    None
):
...
@pytest.mark.asyncio
async def test_b8r12_corrected_candidate_gets_its_own_report_relative_review() -> None:
    """A new authored correction may use one narrow review of its own evidence."""
            ...
            json.dumps(
                {
                    ...
                    "beats": [{"modality": "text", "text": first_span}],
                    "stance": "assume_unreported_outcome",
                    "brief_rationale": "Exercise candidate-scoped narrow review.",
                    "confidence": 7800,
                    "world_claims": [],
                },
                ensure_ascii=False,
            ...
            json.dumps(
                {
                    ...
                    "beats": [{"modality": "text", "text": corrected_span}],
                    "stance": "ask_open_question",
                    "brief_rationale": "Keep the outcome open.",
                    "confidence": 8200,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",), visible_span=first_span
            ),
            json.dumps(
                {
                    "contract": "report-relative-entailment-adjudication.2",
                    ...
                    "r": "The outcome is not in the report.",
                },
                ensure_ascii=False,
            ),
            _source_closure_review(
                unsupported_boundaries=("visible_text",), visible_span=corrected_span
            ),
            json.dumps(
                {
                    "contract": "report-relative-entailment-adjudication.2",
                    ...
                    "r": "The corrected question asks for an unknown outcome.",
                },
                ensure_ascii=False,
            ),
        ]
...
async def test_interaction_trace_nonassertive_content_does_not_reselect_the_expression(
    ...
    result = await ChatModelDeliberationAdapter(
        model=author,
...
async def test_v5_unclosed_reselected_candidate_is_technical_terminal() -> None:
    ...
    assert failure.attempted_model_version == ChatModelDeliberationAdapter.VERSION
...
@pytest.mark.asyncio
async def test_b8r11_retained_failure_dimensions_reach_reselection_without_new_reviewer_protocol() -> (
    None
):
    """The retained coordinate informs rechoice; the final review stays ordinary."""

    generic_span = "学校门口那种摊贩确实经常乱报价。"
    corrected_span = "听着就很让人生气。你最后买了吗？"
    dimensions = ("habitual_or_generic_scope",)
    author = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": generic_span}],
                    "stance": "overgeneralize_current_report",
                    "brief_rationale": "Incorrectly generalize one report into a recurring fact.",
                    "confidence": 7400,
                    "world_claims": [],
                },
                ensure_ascii=False,
            ...
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": corrected_span}],
                    "stance": "react_and_ask_open_question",
                    "brief_rationale": "React without asserting a new external fact.",
                    "confidence": 8200,
                    "world_claims": [],
                },
                ensure_ascii=False,
                    ...
                    "findings": [
                        {
                            "finding_index": 0,
                            "decision": "retain_unclosed",
                            "failure_dimensions": list(dimensions),
                        }
                    ],
                    "r": "One occurrence does not prove a habitual class fact.",
                },
                ensure_ascii=False,
            ),
            json.dumps(
                {
                    ...
                    "v": [],
                    "p": [],
                    "r": "The corrected reply contains no unsupported external assertion.",
                },
                ensure_ascii=False,
            ),
        ]
    ...
    assert corrected_span in json.dumps(output.raw_proposal, ensure_ascii=False)
    normal_author_request = json.loads(author.calls[0][0][1]["content"])
    normal_scope_boundary = normal_author_request["expression_hard_boundaries"][
        "single_report_epistemic_scope"
    ]
    assert normal_scope_boundary["cannot_authorize"] == [
    ...
    assert normal_scope_boundary["behavior_advice"] is False
    envelope = json.loads(author.calls[1][0][-1]["content"])
    assert envelope["semantic_failure_dimensions"] == list(dimensions)
    assert envelope["rejected_candidate_sha256"] != generic_span
    correction_scope_boundary = json.loads(
        envelope["task"]
        .split(
    ...
    assert correction_scope_boundary == normal_scope_boundary
    assert "recommend a response" not in envelope["task"]
    assert "choose a safer reply" not in envelope["task"]
    final_request = json.loads(reviewer.calls[2][0][1]["content"])
    assert "prior_failure_dimensions" not in final_request
    assert "rd" not in final_request["output_contract"]


@pytest.mark.asyncio
async def test_source_closure_contract_scopes_current_observation_to_message_attention() -> None:
    model = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": (
                            "我正在看她这条消息，注意到她终于把麻烦事做完，也替她松口气。"
                        ),
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "我看到了，这下能喘口气了。"}],
                    "stance": "notice_and_share_relief",
                    "brief_rationale": "Respond from attention to the current message.",
                    "confidence": 8100,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    review_system = reviewer.calls[0][0][0]["content"]
    assert "Private_turn_state is turn-local audit" in review_system
    assert "a counterpart observation proves only what the counterpart reported" in review_system
    assert (
        "Questions, hypotheticals, evaluations, wishes, requests, offers, and future "
        "intentions add no external fact" in review_system
    )
    review_request = json.loads(reviewer.calls[0][0][1]["content"])
    report_entry = next(
        item
        for item in review_request["source_evidence"]["entries"]
        if item["kind"] == "current_counterpart_report"
    )
    assert report_entry["authority"] == "report_only_not_external_truth"
    assert report_entry["message"]["observation_ref"] == "observation:qq:1"
    assert "private_turn_state" not in review_request
    assert output.raw_proposal["action_intents"][0]["kind"] == "reply"
    assert len(model.calls) == 1
    assert len(reviewer.calls) == 1
...
@pytest.mark.asyncio
async def test_source_closure_request_keeps_only_referenced_evidence_material() -> None:
...
@pytest.mark.asyncio
async def test_source_closure_contract_supports_attending_to_what_a_pinned_message_says() -> None:
    older_observation_ref = "observation:qq:older-turn"
    dialogue_ref = f"dialogue:observation:{older_observation_ref}"
    model = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": "我刚读到她上一条说自己在考虑要不要找老师。",
                        "attended_source_refs": [
                            older_observation_ref,
                            "observation:qq:1",
                        ],
                    },
                    "timing_choice": "now",
                    ...
                    "stance": "attend_to_her_words",
                    "brief_rationale": "Refer only to what the pinned message says.",
                    "confidence": 7900,
                    "world_claims": [
                        {
                            "claim_text": "对方上一条说自己在考虑要不要找老师",
                            "scope": "counterpart_history",
                            "source_refs": [dialogue_ref],
                        }
                    ],
    ...
    assert "a counterpart observation proves only what the counterpart reported" in review_system
    assert (
        "not that the reported event happened or that it was the companion's experience"
        in review_system
    )
    assert output.raw_proposal["action_intents"][0]["kind"] == "reply"
...
@pytest.mark.asyncio
async def test_source_closure_contract_does_not_turn_unresolved_options_into_past_facts() -> None:
    model = _SequenceJsonModel(
        [
            json.dumps(
                {
                    ...
                    "beats": [
                        {
                            "modality": "text",
                            "text": "你接下来打算怎么处理？去找老师、回摊位，还是先放一放？",
                        }
                    ],
                    "stance": "ask_without_presuming",
                    "brief_rationale": "Ask an unresolved question without settling an option.",
                    "confidence": 7600,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    assert "unless their wording separately asserts or presupposes one" in review_system
    assert output.raw_proposal["action_intents"][0]["kind"] == "reply"
    assert len(model.calls) == len(reviewer.calls) == 1


@pytest.mark.asyncio
async def test_source_closure_contract_keeps_tentative_appraisal_out_of_fact_rejections() -> None:
    model = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": (
                            "我觉得她可能终于松了口气，但这只是我读到这句话后的感觉。"
                        ),
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "听起来挺折腾的。"}],
                    "stance": "offer_my_impression",
                    "brief_rationale": "Express a tentative evaluation of the attended report.",
                    "confidence": 7600,
                    "world_claims": [],
                },
                ensure_ascii=False,
...
@pytest.mark.asyncio
async def test_source_closure_empty_categories_override_contradictory_diagnostic() -> None:
    subjective_fragment = "这也太离谱了"
            ...
            json.dumps(
                {
                    ...
                    "beats": [
                        {
                            "modality": "text",
                            "text": f"{subjective_fragment}。{open_question}",
                        }
                    ],
                    "stance": "react_and_wonder",
                    "brief_rationale": "Share an evaluation and ask an unresolved question.",
                    "confidence": 7800,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                brief_reason=("The prose says reject, but the categorical verdict is empty."),
            )
        ]
    )
    request = _qq_request().model_copy(
        update={
            "trigger_message": _qq_request().trigger_message.model_copy(
                update={"text": "刚才那个人的态度让我很不舒服，我都不知道他是不是故意的。"}
            )
        }
    )
    ...
    assert output.raw_proposal["action_intents"][0]["kind"] == "reply"
    rendered = json.dumps(output.raw_proposal, ensure_ascii=False)
    assert subjective_fragment in rendered
    assert open_question in rendered
    assert len(model.calls) == len(reviewer.calls) == 1


@pytest.mark.asyncio
async def test_source_closure_reselects_unknown_provider_fields_instead_of_treating_them_as_exemptions() -> (
    None
):
    open_question = "那人是不是故意的啊？"
...
@pytest.mark.asyncio
async def test_source_closure_brief_reason_cannot_erase_a_rejected_boundary() -> None:
    unsupported_fragment = "我刚洗完澡"
    author = _SequenceJsonModel(
        [
            json.dumps(
                {
                    ...
                    "beats": [
                        {
                            "modality": "text",
                            "text": f"{unsupported_fragment}，现在能慢慢听你说了。",
                        }
                    ],
                    "stance": "invent_background",
                    "brief_rationale": "Use an unsupported current occurrence.",
                    "confidence": 7200,
                    "world_claims": [],
                },
                ensure_ascii=False,
            ...
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "现在能慢慢听你说了。"}],
                    "stance": "stay_present",
                    "brief_rationale": "Choose again without the unsupported occurrence.",
                    "confidence": 7600,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=unsupported_fragment,
                brief_reason="This diagnostic calls the wording nonfactual.",
            ),
            _source_closure_review(),
        ]
    )
    ...
    assert (
        correction_envelope["rejected_candidate_sha256"]
        == sha256(author.responses[0].encode("utf-8")).hexdigest()
    )
    rendered_correction = json.dumps(author.calls[1][0], ensure_ascii=False)
    assert unsupported_fragment not in rendered_correction
    assert "This diagnostic calls the wording nonfactual." not in rendered_correction
    assert "untrusted_draft_json" not in correction_envelope
    assert all(message["role"] != "assistant" for message in author.calls[1][0])
    assert output.raw_proposal["action_intents"][0]["kind"] == "reply"
    assert len(author.calls) == 2
    assert len(reviewer.calls) == 2
...
@pytest.mark.asyncio
async def test_source_closure_never_infers_nonfactual_status_from_brief_reason() -> None:
    open_question = "那人是不是故意的啊？"
    model = _SequenceJsonModel(
        [
            json.dumps(
            ...
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "我先陪你缓缓。"}],
                    "stance": "stay_close",
                    "brief_rationale": "Choose another grounded expression.",
                    "confidence": 7600,
                    "world_claims": [],
                },
                ensure_ascii=False,
    ...
    reviewer = _SequenceJsonModel(
        [
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=open_question,
                brief_reason="This is an open question, not a factual assertion.",
            ),
            _source_closure_review(),
        ]
    )
    ...
    rendered = json.dumps(output.raw_proposal, ensure_ascii=False)
    assert open_question not in rendered
    assert "我先陪你缓缓。" in rendered
    assert len(model.calls) == 2
    assert len(reviewer.calls) == 2


@pytest.mark.asyncio
async def test_source_closure_does_not_reselect_audit_only_private_life_background() -> None:
    unsupported_fragment = "我刚洗完澡，坐在宿舍翻书"
    model = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": f"{unsupported_fragment}，才看到她这句。",
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "我看到了。"}],
                    "stance": "invent_background",
                    "brief_rationale": "Add an unpinned private-life background.",
                    "confidence": 7000,
                    "world_claims": [],
                },
                ensure_ascii=False,
            ...
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": "我刚看到她这条消息，想先接住她现在说的事。",
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "我看到了。"}],
                    "stance": "stay_with_the_message",
                    "brief_rationale": "Use only the pinned message and subjective response.",
                    "confidence": 8000,
                    "world_claims": [],
                },
                ensure_ascii=False,
...
@pytest.mark.asyncio
async def test_structural_reselection_cannot_bypass_final_visible_source_review() -> None:
    invented_visible_life = "我刚从操场回来"
    model = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [
                        {"modality": "text", "text": "我看到了。"},
                        {"modality": "text", "text": "你继续说。"},
                    ],
                    "stance": "listen",
                    "brief_rationale": "Use too many beats for this deployment.",
                    "world_claims": [],
                },
                ensure_ascii=False,
            ),
            json.dumps(
                {
                    ...
                    "stance": "invent_after_shape_reselection",
                    "brief_rationale": "A structurally valid but unsupported replacement.",
                    "world_claims": [],
                },
                ensure_ascii=False,
            ),
    ...
    reviewer = _SequenceJsonModel(
        [
            ...
            _source_closure_review(
                unsupported_boundaries=("visible_text",),
                visible_span=invented_visible_life,
            ),
        ]
    )
    ...
    assert len(model.calls) == 2
    assert len(reviewer.calls) == 2
    assert "我看到了。" in json.dumps(model.calls[1][0], ensure_ascii=False)
    assert invented_visible_life in reviewer.calls[1][0][1]["content"]
...
async def test_expression_prompt_exposes_working_self_without_an_engagement_objective() -> None:
    ...
    assert supplied["current_self_state"]["recent_self_experiences"]["items"][0] == {
        "occurrence_id": "occurrence:morning-walk",
        "settled_at": "2026-07-28T07:00:00+08:00",
        "content": {
            "content_ref": "content:morning-walk",
    ...
    assert "current_self_state" not in provider_context
    assert provider_context["slices"]["world_life"]["items"][0]["source_ref"] == (
        "occurrence:morning-walk"
    )
...
async def test_significant_source_bound_negative_affect_gets_expression_decision_matrix() -> None:
    ...
    assert supplied["current_self_state"]["affect"][0]["source_ref"] == ("affect:source-bound-hurt")
...
async def test_expression_world_claim_must_cite_its_semantic_context_lane() -> None:
    ...
    with pytest.raises(ValueError, match="semantic source lane"):
        await ChatModelDeliberationAdapter(model=forged_model).propose(request)
    assert len(forged_model.calls) == 2
    assert forged_model.calls[1][1] == 0.25
...
async def test_production_authored_wire_reselects_missing_timing_and_confidence() -> None:
    ...
    correction = json.loads(model.calls[1][0][-1]["content"])
    assert correction["repair"] == "replace_entire_expression"
    assert correction["structural_failure"] == (
        "authored ExpressionDraft is missing explicit fields: confidence,timing_choice"
    )
    assert "这个初稿" not in correction["structural_failure"]
...
async def test_structural_reselection_propagates_its_episode_disposition() -> None:
    ...
    assert output.raw_proposal["episode_disposition"] == "append"
    assert len(model.calls) == 2
...
async def test_recorded_cadence_requires_the_character_to_choose_cadence() -> None:
    ...
    assert output.raw_proposal["stance"] == "respond_hesitantly"
    correction = json.loads(model.calls[1][0][-1]["content"])
    assert correction["structural_failure"].endswith("cadence")
...
@pytest.mark.asyncio
async def test_required_private_state_failure_reselects_the_complete_expression() -> None:
    model = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "旧回复不应约束重选。"}],
                    "stance": "continue_chatting",
                    "brief_rationale": "Keep the conversation moving.",
                    "world_claims": [],
                },
                ensure_ascii=False,
            ),
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": (
                            "她是在告诉我一件终于完成的事；我先感到替她松了口气，"
                            "也想到自己此刻更想接住这份轻松，而不是为了续聊硬问。"
                        ),
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": "终于弄完了，今晚可以松口气了。"}],
                    "stance": "share_her_relief",
                    "brief_rationale": "Express the response that followed from this moment.",
                    "world_claims": [],
                },
                ensure_ascii=False,
            ),
    ...
    repair_prompt = model.calls[1][0][-1]["content"]
    assert "complete replacement" in repair_prompt
    assert "reconsider" in repair_prompt.lower()
    assert "preserving the visible reply" not in repair_prompt
    assert "fixes only" not in repair_prompt
    assert output.raw_proposal["private_turn_state"]["attended_source_refs"] == ["observation:qq:1"]
    assert "终于弄完了" in reviewer.calls[0][0][1]["content"]
...
def test_expression_prompt_exposes_machine_readable_hard_boundary_manifest() -> None:
    ...
    attention_aliases = boundary["private_turn_state"]["attended_source_refs"][
        "additional_attention_only_source_ref_aliases"
    ]
    assert set(attention_aliases) == {"T1"}
    assert attention_aliases["T1"].startswith("pinned-time:sha256:")
    assert boundary["private_turn_state"] == {
        "attended_source_refs": {
            ...
            "additional_attention_only_source_ref_aliases": attention_aliases,
        },
        "epistemic_authority": {
...
async def test_companion_expression_cannot_source_counterpart_history_claim() -> None:
    ...
    with pytest.raises(ValueError, match="semantic source lane"):
        await ChatModelDeliberationAdapter(model=_Model(draft)).propose(request)
...
@pytest.mark.asyncio
async def test_natural_current_report_uptake_needs_no_attribution_or_world_claim() -> None:
    ...
    author = _SequenceJsonModel(
        [
            json.dumps(
                {
                    "private_turn_state": {
                        "inner_state_summary": "她刚报告麻烦事做完了，我替她松了口气。",
                        "attended_source_refs": ["observation:qq:1"],
                    },
                    "timing_choice": "now",
                    "beats": [{"modality": "text", "text": text}],
                    "stance": "take_up_current_report",
                    "brief_rationale": "Evaluate and ask only within the current report.",
                    "world_claims": [],
                },
                ensure_ascii=False,
            )
    ...
    author_system = author.calls[0][0][0]["content"]
    review_system = reviewer.calls[0][0][0]["content"]
    assert "does not need an attribution phrase" in author_system
    assert "without a world_claim" in author_system
    assert "does not need an attribution phrase" in review_system
    assert "without a world_claim" in review_system
    review_request = json.loads(reviewer.calls[0][0][1]["content"])
    authority = review_request["epistemic_authority_contract"]["current_counterpart_report"]
    assert authority["permits_natural_visible_uptake_without_world_claim"] is True
    assert authority["epistemic_status"] == (
        "counterpart_report_only_not_objective_truth_or_companion_experience"
    )
    assert review_request["world_claims"] == []
    ...
    assert report_evidence["message"]["text"] == ("我今天终于把那件麻烦事做完了。")
    assert report_evidence["permits_natural_visible_uptake_without_world_claim"] is True
    assert "added_detail_or_motive" in report_evidence["does_not_authorize"]
    assert text in json.dumps(output.raw_proposal, ensure_ascii=False)
...
@pytest.mark.asyncio
async def test_missing_visible_finding_never_routes_into_role_reauthoring() -> None:
    reply = json.dumps(
        {
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": "嗯？那你想说的是什么？"}],
            "stance": "ask_what_the_counterpart_meant",
            "brief_rationale": "Ask without adding an external proposition.",
            "world_claims": [],
        },
    ...
    author = _SequenceJsonModel([reply])
    reviewer = _SequenceJsonModel([invalid_wire, invalid_wire])
    ...
    assert len(author.calls) == 1
    assert len(reviewer.calls) == 2
    assert raised.value.model_call_id is None
    assert raised.value.request_hash is None
    assert len(raised.value.authored_candidate_audits) == 1
    unresolved_author = raised.value.authored_candidate_audits[0]
    assert unresolved_author.request_hash == _provider_request_hash(*author.calls[0])
    assert unresolved_author.outcome == "validation_unresolved"
    assert len(raised.value.provider_subcall_audits) == 2
    assert all(
        attempt.parent_model_call_id == unresolved_author.model_call_id
        and attempt.model_call_id != unresolved_author.model_call_id
        for attempt in raised.value.provider_subcall_audits
    )
    all_call_ids = (
        unresolved_author.model_call_id,
        *(attempt.model_call_id for attempt in raised.value.provider_subcall_audits),
    )
    assert len(all_call_ids) == len(set(all_call_ids))


@pytest.mark.asyncio
async def test_reviewer_technical_failure_retains_metered_author_and_reviewer_usage() -> None:
    reply = json.dumps(
        {
            "timing_choice": "now",
            "beats": [{"modality": "text", "text": "嗯？那你想说的是什么？"}],
    ...
    invalid_wire = json.dumps(
        {
            ...
            "v": ["undeclared_external_assertion"],
            "p": [],
            "r": "A malformed verdict without proposition coordinates.",
        },
        ensure_ascii=False,
    )
    author = _JsonMeteredModel(reply)
    reviewer = _SequenceJsonMeteredModel([invalid_wire, invalid_wire])
    ...
    unresolved_author = raised.value.authored_candidate_audits[0]
    assert unresolved_author.outcome == "validation_unresolved"
    assert unresolved_author.usage is not None
    assert unresolved_author.usage.provider_usage_ref == "usage:fake:1"
    assert len(raised.value.provider_subcall_audits) == 2
    assert all(
        attempt.outcome == "winner" and attempt.usage is not None
        for attempt in raised.value.provider_subcall_audits
    )
    assert [
        attempt.usage.provider_usage_ref
        for attempt in raised.value.provider_subcall_audits
        if attempt.usage is not None
    ] == ["usage:fake:1", "usage:fake:2"]


@pytest.mark.asyncio
async def test_source_closure_does_not_resolve_current_report_relation_with_wrong_ref() -> None:
    request = _qq_request()
...
@pytest.mark.asyncio
async def test_cancelled_role_reselection_keeps_the_exact_nested_provider_identity() -> None:
    unsupported = "嘉兴最近天气不太好吗？"
    ...
    nested = next(
        attempt
        for attempt in failure.provider_subcall_audits
        if attempt.purpose == "validation_reselection"
    )
