...
def _proposal(index: int = 1) -> dict[str, object]:
    return {
        ...
        "proposal_kind": "fact_transition",
        "authority_contract_ref": "proposal-contract:fact.1",
        "change_id": f"change:{index}",
        "proposed_change_hash": format(index % 16, "x") * 64,
        "mutation_event_type": "FactCommitted",
    }
...
def _effect(
    ...
    *,
    ...
    if role == "domain_mutation":
        value.update(change_id=change_id, accepted_change_hash=change_hash)
    return value
...
def test_domain_effect_requires_one_proposal_and_complete_change_authority() -> None:
    ...
    effect["proposal_refs"] = ("proposal:1", "proposal:2")
...
def test_domain_effect_must_match_exact_proposal_authority(field: str, value: str) -> None:
    raw = _manifest_raw(status="accepted", effects=(_effect(),))
    effect = dict(raw["authorized_effects"][0])  # type: ignore[index]
    effect[field] = value
    raw["authorized_effects"] = (effect,)
...
@pytest.mark.parametrize("case", ["budget_only", "missing", "duplicate"])
def test_accepted_manifest_requires_one_domain_effect_per_proposal(case: str) -> None:
    if case == "budget_only":
        effects = (_effect(role="budget_reservation", event_type="BudgetReserved"),)
        proposals = (_proposal(1),)
    elif case == "missing":
        effects = (_effect(),)
        proposals = (_proposal(1), _proposal(2))
    else:
        effects = (_effect(), _effect(1))
        proposals = (_proposal(1),)
    raw = _manifest_raw(status="accepted", proposals=proposals, effects=effects)

    with pytest.raises(AcceptanceManifestError) as captured:
        parse_acceptance_manifest_v2(raw, accepted_integration_enabled=True)

    assert _error_code(captured.value) == "acceptance_manifest.domain_authority_incomplete"
...
def test_change_id_is_unique_even_when_hashes_differ() -> None:
    second = _proposal(2)
    second["change_id"] = "change:1"
    raw = _manifest_raw(proposals=(_proposal(1), second))

    with pytest.raises(AcceptanceManifestError) as captured:
        parse_acceptance_manifest_v2(raw)

    assert _error_code(captured.value) == "acceptance_manifest.duplicate_change_id"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda raw: raw["proposals"][0].update(change_id="change:tampered"),
...
def test_parser_revalidates_model_construct_and_manifest_reference() -> None:
    ...
    reference_values.update(
        acceptance_event_ref="event:acceptance:1", accepted_at_world_revision=13
    )
...
def test_proposals_effects_and_refs_must_be_sorted_unique_and_contiguous() -> None:
    ...
    with pytest.raises(ValidationError, match="noncanonical_proposal_refs"):
        AcceptanceAuthorizedEffectV2(**_effect(proposal_refs=("proposal:2", "proposal:1")))
...
def test_effect_cannot_reference_a_proposal_outside_manifest() -> None:
    ...
    with pytest.raises(AcceptanceManifestError) as captured:
        parse_acceptance_manifest_v2(raw, accepted_integration_enabled=True)
    assert _error_code(captured.value) == "acceptance_manifest.unknown_effect_proposal"
...
def test_parser_preflights_dos_limits_before_model_validation(target: str) -> None:
    ...
    if target == "proposals":
        ...
        raw = _manifest_raw(status="accepted", effects=(_effect(),))
        effect = dict(raw["authorized_effects"][0])  # type: ignore[index]
        effect["proposal_refs"] = tuple(
            f"proposal:{index}" for index in range(MAX_EFFECT_PROPOSAL_REFS + 1)
        )
        raw["authorized_effects"] = (effect,)
...
def test_proposal_value_object_rejects_non_hex_authority_hash() -> None:
    with pytest.raises(ValidationError, match="invalid_digest"):
        AcceptanceManifestProposalV2(**{**_proposal(), "proposed_change_hash": "z" * 64})
