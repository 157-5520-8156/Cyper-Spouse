...
def test_only_the_active_lease_owner_can_complete_a_trigger() -> None:
    ...
    ledger.commit(
        [event("event-claim", "TriggerProcessClaimed", {
    ...
    with pytest.raises(ValueError, match="does not own"):
        ledger.commit(
            [event("event-complete-wrong-owner", "TriggerProcessCompleted", {
                "trigger_id": "trigger-1",
...
def test_expired_trigger_can_be_reclaimed_with_preserved_attempt_lineage() -> None:
    ...
    ledger.commit(
        [event("event-claim", "TriggerProcessClaimed", {
            "process": first.model_dump(mode="json")
        })],
        expected_world_revision=0,
        expected_deliberation_revision=0,
    )
    ledger.commit(
        [event("event-reclaim", "TriggerProcessReclaimed", {
    ...
    ledger.commit(
        [event("event-complete", "TriggerProcessCompleted", {
            "trigger_id": "trigger-1",
