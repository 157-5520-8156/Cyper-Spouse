...
def test_fact_memory_draft_normalizes_probability_float_salience_to_bp() -> None:
    raw = _retained()
    raw["salience"] = {
        key: value / 10_000
        for key, value in raw["salience"].items()  # type: ignore[union-attr]
    }

    result = materialize_fact_memory_draft(json.dumps(raw))
    assert result is not None
    assert result.salience.autobiographical_relevance_bp == 6800
    assert result.salience.future_utility_bp == 7900
    assert result.salience.matrix_version == "memory-salience-matrix.1"
...
def test_fact_memory_draft_rejects_unbounded_or_incomplete_model_output(
    raw: dict[str, object],
) -> None:
