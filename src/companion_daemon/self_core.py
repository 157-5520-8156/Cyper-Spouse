...
class SelfCore:
    ...
    def initial(cls, character_name: str = "沈知栀") -> SelfCore:
        return cls(
            identity=f"我叫{character_name}；稳定身份由角色档案约束，尚无新增的可验证个人经历。",
            user_profile="还没有足够的已验证用户信息。",
...
静态角色档案会在别处提供姓名、学校和相识设定；这里不要重述、补写或猜测那些内容。
请按以下格式输出（每行一个部分，用 --- 分隔）：

[动态自我认知：只写聊天后新增、且在记忆里有来源的个人变化；没有就明确说没有]
---
[你对用户的了解：来自记忆的真实信息]
---
[你们的关系现状：聊过什么、关系阶段]
