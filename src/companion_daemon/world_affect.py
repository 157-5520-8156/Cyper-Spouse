...
from companion_daemon.emotion_programs import EmotionProgramInput, evaluate_emotion_program
...
class AffectOutcome:
    ...
    repair_target_reference: str = ""
    repair_evidence_references: tuple[str, ...] = ()
    core_affect: dict[str, object] | None = None
    active_episodes: tuple[dict[str, object], ...] = ()
    archived_episodes: tuple[dict[str, object], ...] = ()
...
def apply_appraisal(
    ...
    *,
    ...
    repair_target_reference = str(current.get("repair_target_reference") or "")
    raw_evidence_references = current.get("repair_evidence_references", ())
    repair_evidence_references = (
        tuple(str(item) for item in raw_evidence_references if str(item))
        if isinstance(raw_evidence_references, (list, tuple))
        else ()
    )
    ...
    evidence_reference = source_reference or f"logical:{logical_at}"
    duplicate_repair_evidence = (
        appraisal == "boundary_respected" and evidence_reference in repair_evidence_references
    )
    has_affect_effect = appraisal in _EFFECTS and not duplicate_repair_evidence
    for key, delta in (_EFFECTS.get(appraisal, {}) if has_affect_effect else {}).items():
        scaled = delta
    ...
    archived_episodes = _episode_items(current.get("archived_episodes", ()))
    if has_affect_effect:
        new_episode = _new_episode(
            appraisal=appraisal,
        ...
        if appraisal in {
            ...
            harm_half_lives = [
                int(item.get("half_life_hours") or 1)
                for item in episodes
                if int(item.get("valence") or 0) < 0 and str(item.get("target") or "") == target
            ]
            if harm_half_lives:
                new_episode["half_life_hours"] = max(
                    int(new_episode["half_life_hours"]), max(harm_half_lives)
                )
        episodes = (
            *episodes,
            new_episode,
        )
        if episodes and (raw_episodes or _vector(current) == baseline):
            vector = _episode_vector(episodes, baseline, fallback=vector)
    ...
    unresolved = _negative_total(vector, baseline) >= 8
    if appraisal in HARMFUL_INTERACTION_APPRAISALS:
        ...
        repair_evidence_count = 0
        repair_target_reference = source_reference
        repair_evidence_references = ()
        ...
        if not duplicate_repair_evidence:
            repair_evidence_references = (*repair_evidence_references, evidence_reference)
            repair_evidence_count = min(6, len(repair_evidence_references))
        behavior_tendency = "repair_observing"
    ...
    return AffectOutcome(
        ...
        source_appraisal=(
            appraisal if has_affect_effect else str(current.get("source_appraisal") or appraisal)
        ),
        reason=reason,
        ...
        repair_target_reference=repair_target_reference,
        repair_evidence_references=repair_evidence_references,
        profile=profile,
    )
...
def decay_affect(
    ...
    baseline = _baseline(current)
    accumulated_seconds = max(0, int(elapsed_seconds)) + int(
        current.get("decay_remainder_seconds") or 0
    )
    hours, remainder_seconds = divmod(accumulated_seconds, 3600)
    episodes, newly_archived = _decay_episodes(current, elapsed_seconds, logical_at)
    archived_episodes = (
        *_episode_items(current.get("archived_episodes", ())),
        *newly_archived,
    )
    raw_episodes = current.get("active_episodes", ())
    episodes_are_authoritative = (
        isinstance(raw_episodes, (list, tuple))
        and bool(raw_episodes)
        and all(
            isinstance(item, dict) and isinstance(item.get("affect_vector"), dict)
            for item in raw_episodes
        )
    )
    vector = (
        _episode_vector(episodes, baseline, fallback=_vector(current))
        if episodes_are_authoritative
        else _vector(current)
    )
    ...
    if current_repair_quality and repair_evidence_count < int(profile["repair_evidence_required"]):
    ...
    if (
        ...
        episodes = tuple(retained)
        archived_episodes = (*archived_episodes, *resolved)
    if repair_observation_seconds:
        ...
        mode, expression = (
            ("guarded", "guarded") if behavior_tendency == "withdraw" else ("patient", "neutral")
        )
    else:
    ...
    return AffectOutcome(
        ...
        repair_evidence_count=repair_evidence_count,
        repair_target_reference=str(current.get("repair_target_reference") or ""),
        repair_evidence_references=tuple(
            str(item) for item in current.get("repair_evidence_references", ()) if str(item)
        )
        if isinstance(current.get("repair_evidence_references", ()), (list, tuple))
        else (),
        core_affect=_core_affect(vector, baseline, episodes),
        active_episodes=episodes,
        archived_episodes=archived_episodes,
...
def outcome_payload(
    outcome: AffectOutcome, *, logical_at: str, event_type: str
) -> dict[str, object]:
    payload = {
        ...
        "repair_target_reference": outcome.repair_target_reference,
        "repair_evidence_references": list(outcome.repair_evidence_references),
        "core_affect": dict(outcome.core_affect or {}),
        "active_episodes": [dict(item) for item in outcome.active_episodes],
        "archived_episodes": [
            dict(item)
            for item in sorted(
                outcome.archived_episodes,
                key=lambda item: (
...
def affect_guidance(state: dict[str, Any]) -> str:
    ...
    if behavior == "withdraw":
        return (
            "情绪还没过去：可以短一点、慢一点，说明不舒服，但不辱骂、不威胁，也不用冷处理惩罚对方。"
        )
    if behavior == "guarded":
...
def _vector(current: dict[str, object]) -> dict[str, int]:
    ...
    return {
        key: _clamp(int(raw.get(key, 0))) if isinstance(raw, dict) else 0 for key in AFFECT_KEYS
    }
...
def _new_episode(
    *,
    ...
    for key, delta in effect.items():
        ...
        if intensity is not None:
            if 1 <= raw_intensity <= 4:
                ...
                scaled = int(round(delta * (50 + episode_intensity) / 100))
        if scaled:
            affect_vector[key] = scaled
    program = evaluate_emotion_program(
        EmotionProgramInput(
            event=appraisal,
            agency=str(dimensions.get("agency") or "unknown"),  # type: ignore[arg-type]
            target=str(dimensions.get("program_target") or target),
            certainty=int(dimensions.get("certainty", 100)),
            goal_congruence=int(dimensions.get("goal_congruence", 0)),
    ...
    if appraisal in HARMFUL_INTERACTION_APPRAISALS:
        half_life += min(
            12,
            learned_resentment * int(profile["resentment_half_life_gain_hours"]),
        )
        episode_intensity = min(
            100,
            episode_intensity
    ...
    return {
        ...
        "initial_intensity": episode_intensity,
        "affect_vector": affect_vector,
        "current_affect_vector": dict(affect_vector),
        "component_half_life_hours": component_half_lives,
        "valence": valence,
        ...
        "emotion_program": {
            ...
            "components": dict(program.components),
...
def _decay_episodes(
    ...
    for raw in raw_episodes:
        ...
        program = raw.get("emotion_program")
        program = program if isinstance(program, dict) else {}
        process_effects = program.get("process_effects")
        process_effects = process_effects if isinstance(process_effects, dict) else {}
        decay_multiplier = max(
            0.25, min(1.5, float(process_effects.get("decay_multiplier") or 1.0))
        )
        elapsed_hours = _hours_between(str(raw.get("started_at") or ""), logical_at)
        initial_vector = raw.get("affect_vector")
        ...
        current_vector = {
            key: int(
                round(
                    ...
                    * pow(
                        0.5,
                        (elapsed_hours * decay_multiplier)
                        / max(1.0, float(component_half_lives.get(key) or half_life)),
                    )
                )
            )
            for key, value in initial_vector.items()
            if int(value) != 0
        }
        ratios = [
            abs(current_vector.get(key, 0) / int(value))
            for key, value in initial_vector.items()
            if int(value) != 0
        ]
        intensity = int(round(int(raw.get("initial_intensity") or 0) * max(ratios, default=0)))
        if intensity < 8:
        ...
        decayed.append(
            {
                **raw,
                "intensity": intensity,
                "current_affect_vector": current_vector,
                "updated_at": logical_at,
                "status": "regulated"
                if elapsed_seconds > 0
                else str(raw.get("status") or "active"),
            }
        )
    return tuple(decayed), tuple(archived)
...
def _episode_vector(
    ...
    *,
    ...
    """Project the observable vector from sourced active episodes.

    Old event ledgers may contain episodes written before component vectors
    existed.  They retain their recorded projection until a migration can
    rewrite them as sourced episodes; new ledgers are rebuilt solely from the
    episode contributions.
    """

    if any(not isinstance(item.get("affect_vector"), dict) for item in episodes):
        return dict(fallback)
    totals = {key: int(baseline[key]) for key in AFFECT_KEYS}
    for episode in episodes:
        initial_intensity = max(
            1, int(episode.get("initial_intensity") or episode.get("intensity") or 1)
        )
        current_intensity = max(0, int(episode.get("intensity") or 0))
        scale = current_intensity / initial_intensity
        contribution = episode.get("current_affect_vector", episode["affect_vector"])
        assert isinstance(contribution, dict)
        for key in AFFECT_KEYS:
            if "current_affect_vector" in episode:
                totals[key] += int(contribution.get(key, 0))
            else:
                totals[key] += int(round(int(contribution.get(key, 0)) * scale))
    return {key: _clamp(value) for key, value in totals.items()}


def _profile(value: object) -> dict[str, object]:
    result = dict(DEFAULT_AFFECT_PROFILE)
    result["emotion_half_life_hours"] = dict(
        DEFAULT_AFFECT_PROFILE["emotion_half_life_hours"]  # type: ignore[arg-type]
    )
    if isinstance(value, dict):
        for key, default in DEFAULT_AFFECT_PROFILE.items():
            ...
            elif key == "emotion_half_life_hours" and isinstance(value[key], dict):
                component_profile = dict(result[key])
                for emotion, hours in value[key].items():
                    if emotion in AFFECT_KEYS:
                        component_profile[str(emotion)] = max(1, min(720, int(hours)))
                result[key] = component_profile
            elif isinstance(default, int):
                result[key] = max(1, min(168, int(value[key])))
    return result


def _episode_items(value: object) -> tuple[dict[str, object], ...]:
...
def _archive_episode(
    ...
    return {
        **episode,
        "status": "resolved",
        "resolved_at": logical_at,
        "resolution_reason": reason,
    }


def _decay_resolution_at(
    episode: dict[str, object], logical_at: str, decay_multiplier: float
) -> str:
    """Return the step-size-independent instant an episode crosses archive level."""
    try:
        started = datetime.fromisoformat(str(episode.get("started_at") or ""))
    except ValueError:
        return logical_at
    initial = max(0.0, float(episode.get("initial_intensity") or 0))
    if initial <= 7.5:
    ...
    initial_vector = initial_vector if isinstance(initial_vector, dict) else {}
    relevant_half_lives = [
        float(component_half_lives.get(key) or episode.get("half_life_hours") or 12)
        for key, value in initial_vector.items()
        if int(value) != 0
    ]
    slowest_half_life = max(
        relevant_half_lives,
    ...
    return min(resolved, observed).isoformat()


def _hours_between(started_at: str, logical_at: str) -> float:
    try:
        return max(
            0.0,
