...
async def lifespan(_app: FastAPI):
    ...
    # Runtime ownership is a deployment property, never inferred from a
    # module-global object identity.  Default/factory World-v2 apps therefore
    # cannot close (or accidentally resolve) the archived Engine; only an
    # explicitly composed archive app owns that compatibility lifecycle.
    if bool(getattr(_app.state, "owns_archive_engine", False)):
        await engine.aclose()
...
class _LazyArchiveEngine:
    ...
    The explicit ``archive_app`` exposes archive/debug routes while their
    migration is incomplete.  Importing the default World-v2 ASGI entry must
    never construct a second runtime authority, so the proxy resolves only
    when an archive handler dereferences it.  Tests may continue to replace
    the public ``engine`` binding with a concrete fixture.
    """
...
def create_http_asgi_app(
    *,
    ...
    configured.state.owns_archive_engine = False
    return configured
...
async def health(request: Request) -> dict[str, str]:
    ...
    if (
        not bool(getattr(request.app.state, "owns_archive_engine", False))
        and _existing_http_v2_capture(request.app) is None
    ):
...
def world_command(world_id: str, request: WorldCommandRequest) -> dict[str, object]:
    ...
    try:
        decision = WorldKernel(engine.store).submit(
...
def world_rebuild(world_id: str) -> dict[str, object]:
    try:
        return (
            WorldKernel(engine.store).rebuild_projection(world_id, "world_current_state").__dict__
        )
    except WorldError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def reconcile_unknown_delivery(
    ...
    if selected_segment_id:
        ...
        if segment.get("status") == "delivered":
            if (
                ...
                or str(segment.get("external_receipt") or "") != request.external_receipt.strip()
            ):
                raise HTTPException(
                    status_code=409,
        ...
        unresolved_segments = [
            item
            for item in segments
            if isinstance(item, dict) and item.get("status") in {"planned", "unknown"}
        ]
    ...
    if current_status != "unknown":
        if current_status == request.status:
            ...
            stored_receipt = (
                str(result.get("external_receipt") or "") if isinstance(result, dict) else ""
            )
    ...
    if kernel.revision(world_id) != request.expected_revision:
        raise HTTPException(
            status_code=409,
            detail=(
                f"expected revision {request.expected_revision}, got {kernel.revision(world_id)}"
            ),
        )
...
def _compose_asgi_app(
    *,
    ...
    owns_archive_engine: bool,
) -> FastAPI:
    """Build one authority-specific ASGI graph from the declaration catalog.

    Default and archive ingress are complementary sets.  In particular,
    ``/messages`` and the World-v2 scheduler/operator routes never coexist
    with ``/qq/webhook`` or any legacy writer in one deployed app.
    """
    ...
    configured.state.http_v2_capture = None
    configured.state.dashboard_session_secret = secrets.token_bytes(32)
    configured.state.owns_archive_engine = owns_archive_engine
    return configured


# Freeze the declaration catalog before rebinding the supported ASGI entries.
_route_catalog_app = app
archive_app = _compose_asgi_app(
    source=_route_catalog_app,
    title="Girl Agent Companion Daemon (archive)",
    include_default_v2_routes=False,
    owns_archive_engine=True,
)
app = _compose_asgi_app(
    source=_route_catalog_app,
    title="Girl Agent Companion Daemon (World v2)",
    include_default_v2_routes=True,
    owns_archive_engine=False,
)


def create_archive_asgi_app() -> FastAPI:
    """Return an explicit archive-only composition, disabled by default.

    The historical source and persisted data remain intact, but callers must
    choose this factory (or ``archive_app``) deliberately.  No World-v2
    message, scheduler, dashboard, or room ingress is registered here.
    """

    return _compose_asgi_app(
        source=_route_catalog_app,
        title="Girl Agent Companion Daemon (archive)",
        include_default_v2_routes=False,
        owns_archive_engine=True,
