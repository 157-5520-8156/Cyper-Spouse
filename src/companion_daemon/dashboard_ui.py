...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    body.canvas-only { margin:0; overflow:hidden; background:#141115; }
    body.canvas-only .bar, body.canvas-only aside, body.canvas-only .game-chrome,
    body.canvas-only .scene-info, body.canvas-only .key { display:none; }
    body.canvas-only .wrap, body.canvas-only .game-frame { display:block; width:100vw; height:100vh; margin:0; padding:0; border:0; box-shadow:none; }
    body.canvas-only #roomCanvas { display:block; width:100vw; height:100vh; max-height:none; margin:0; }
  </style>
</head>
<body>
  ...
  <script>
    ...
    async function init() {
      ...
      try {
        ...
        if (!response.ok) throw new Error(`用户列表读取失败 (${response.status})`);
        const users = await response.json();
        select.innerHTML = (users.users.length ? users.users : ['geoff']).map(u => `<option value="${esc(u)}">${esc(u)}</option>`).join('');
      } catch (_) {
        // The visual home remains usable for its non-persistent previews
        // while the daemon is stopped or temporarily unavailable.
    ...
    async function applyPreviewMode() {
      ...
      document.getElementById('gameAction').textContent = preview.gameAction;
    }
