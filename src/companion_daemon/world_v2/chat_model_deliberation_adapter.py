...
def expression_draft_shape_contract(*, include_world_claims: bool = True) -> str:
    ...
    contract = (
        ...
        "and attended_source_refs as zero to eight unique refs copied only from Context when "
        "the person actually noticed them. Free first-person mental material is allowed, but "
...
class SourceClosureReselectionLane:
    """One stronger role executor plus the independent review route for its draft.

    This value keeps authorship and fact authority separate without exposing
    four loosely related constructor arguments at every cognition seam.  It is
    used only after an ordinary authored candidate has failed source closure;
    timing, motive, wording, and silence remain the role model's decision.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            if self.report_relative_reviewer is not self.reviewer:
                raise ValueError(
...
class CompanionIdentityFrame(FrozenModel):
    ...
    # Compatibility-only input for older fixtures. Current relationship is a
    # time-varying World projection and therefore cannot enter the stable
    # identity prompt, its hash, or source-closure authority.
    relationship_frame: str | None = Field(
        default=None,
        min_length=1,
        max_length=512,
        exclude=True,
...
def companion_identity_source_ref(
    ...
    *,
    ...
    """Return one immutable token for one explicitly configured semantic lane."""

    if scope == "stable_identity":
        # Preserve the established stable-identity token across the typed
        # history-lane addition. Those facts have separate refs below and
        # must not silently widen the authority of this legacy token.
...
def companion_identity_source_refs(
    ...
    """Expose immutable deployment facts that may authorize visible claims.

    ``counterpart_history_facts`` is retained only as legacy historical
    context.  A deployment note about the counterpart is not a supersedable
    UserFact and therefore must never authorize a current counterpart claim.
    """

    refs = {
        "stable_identity": companion_identity_source_ref(identity),
    }
    ...
    async def report_relative_review_once() -> SourceClosureReviewResult:
        ...
        try:
            result = await _adjudicate_report_relative_findings(
                ...
                allow_private_role_reclassification=(
                    candidate_inventory_decomposition is None
                ),
                invalid_wire=report_relative_invalid_wire,
                captured_provider_result=captured,
            )
...
# Backward-compatible internal name retained while every production caller is
# migrated to candidate-wide coverage.
    ...
    async def _complete(
        ...
        *,
        ...
        exact_request_emission = bool(
            getattr(self._model, "reports_exact_request_emission", False)
        )
    ...
    def _model_led_messages(
        ...
        *,
        ...
        system = (
            ...
            "matching authority. Ordinary background or phenomenological generalizations whose "
        ...
        current_self_state = provider_context.pop("current_self_state", None)
        if current_self_state is not None:
            # Keep the role model's working perspective at the same envelope
            # level as the current message instead of burying it inside a
            # second JSON string.  The remaining Context still carries every
            # copyable source ref, and the full Capsule remains the sole
        ...
        if current_self_state is not None:
            user_material["current_self_state"] = current_self_state
...
# Kept private as a source-compatible alias for older test and extension code.
_parse_character_recall_request = parse_character_recall_request
