  ...
  class DashboardRoomEditor {
    ...
    mount() {
      ...
      this.panel.innerHTML = `
        ...
        <label>家具<select data-field="object"></select></label>
        <label>显示模式<select data-field="mode"><option value="composite">最终合成</option><option value="master">仅母版</option><option value="alpha">选中 alpha</option></select></label>
    ...
    bind() {
      ...
      objectSelect.addEventListener('change', event => { this.selectedId = event.target.value; this.refresh(); this.redraw(); });
      this.panel.querySelector('[data-field="mode"]').addEventListener('change', event => { this.mode = event.target.value; this.redraw(); });
      ...
      for (const axis of ['x', 'y']) this.panel.querySelector(`[data-field="${axis}"]`).addEventListener('input', event => {
        ...
        this.object.frontOccluder.origin[index] = Math.round(Number(event.target.value) || 0);
        this.refreshSnippet();
        this.redraw();
      });
      for (const side of ['behind', 'front']) this.panel.querySelector(`[data-action="${side}"]`).addEventListener('click', () => {
        this.runtime.activatePreview(new URLSearchParams(`demo=audit&object=${this.selectedId}&side=${side}`));
        this.redraw();
      });
    ...
    bounds() {
      const object = this.object, image = this.runtime.images[object.frontOccluder.image], stage = this.runtime.stage;
      if (!image) return null;
      const [x, y] = object.frontOccluder.origin;
    ...
    pointerDown(event) {
      ...
      this.drag = {x, y, origin:[...this.object.frontOccluder.origin]};
    ...
    pointerMove(event) {
      ...
      this.object.frontOccluder.origin = [
    ...
    refresh() {
      ...
      this.panel.querySelector('[data-field="x"]').value = object.frontOccluder.origin[0];
      this.panel.querySelector('[data-field="y"]').value = object.frontOccluder.origin[1];
    ...
    manifestSnippet() {
      return JSON.stringify({id:this.object.id, frontOccluder:{origin:this.object.frontOccluder.origin}}, null, 2);
    }
    ...
    draw() {
      ...
      if (this.visibility.footprints) for (const object of this.runtime.scene.objects) for (const point of object.footprint) this.drawTile(point, 'rgba(210,80,90,.18)', 'rgba(255,120,120,.7)');
      ...
      if (this.visibility.depth) {
        ctx.save(); ctx.fillStyle = '#ffe27b'; ctx.font = '10px Pixel,sans-serif'; ctx.fillText(`${object.id} depth=${this.runtime.depthKey(object.tile, object.frontOccluder.depthBias)}`, dx+5, dy-5);
    ...
    drawAlpha() {
      const object = this.object, image = this.runtime.images[object.frontOccluder.image], bounds = this.bounds();
