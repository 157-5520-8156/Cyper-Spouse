...
_ASSISTANTESE_PATTERNS = [
    ...
    re.compile(r"我之前看(?:一个|个)?(?:朋友|同学|学长|学姐)[^。！？]{0,36}(?:照片|发|拍)[^。！？]{0,48}(?:。|！|？)?"),
    re.compile(r"我(?:一个|有个|有一个)?(?:高中同学|大学同学|同学|朋友)[^。！？]{0,36}(?:在那儿|在那|在你们学校|读过|上过)[^。！？]{0,36}(?:。|！|？)?"),
    re.compile(r"(?:不过|但是|而且|然后)?我?(?:朋友|同学|室友|舍友)(?:也|之前|跟我|和我|说|发|拍)[^。！？]{0,48}(?:。|！|？)?"),
    ...
    re.compile(r"[^。！？]{0,16}(?:不算)?白跑[^。！？]{0,16}(?:。|！|？)?"),
    re.compile(r"[^。！？]{0,16}一起迟到[^。！？]{0,16}(?:。|！|？)?"),
    re.compile(r"[^。！？]{0,16}有点亏[^。！？]{0,16}(?:。|！|？)?"),
