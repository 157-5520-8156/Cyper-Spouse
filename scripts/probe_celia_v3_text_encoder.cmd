...
set OUTPUT=%ROOT%\training\runs\celia-v3-v2-clean-text-encoder-probe

powershell -NoProfile -ExecutionPolicy Bypass -File "%ROOT%\scripts\train_celia_v3_pilot.ps1" -Steps 20 -Root "%ROOT%" -PythonPath "%PYTHON%" -BaseModelPath "%MODEL%" -DatasetDir "%ROOT%\assets\reference" -OutputDir "%OUTPUT%" -TrainTextEncoder
