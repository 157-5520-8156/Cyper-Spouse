...
HEALTH_PORT="${LOCAL_APPRAISAL_PORT:-8188}"
HEALTH_MODEL="${LOCAL_APPRAISAL_MODEL:-mlx-community/Qwen3-1.7B-4bit}"
...
INFERENCE_FAILURE_FILE="${TMPDIR:-/tmp}/girl-agent-local-appraisal-watchdog.failures"
CAPACITY_DIR="${LOCAL_APPRAISAL_CAPACITY_DIR:-${TMPDIR:-/tmp}/girl-agent-local-appraisal.capacity}"
CAPACITY_STATE="${CAPACITY_DIR}/state"
CAPACITY_PYTHON="${LOCAL_APPRAISAL_PYTHON:-/usr/bin/python3}"
THROTTLE_SECONDS=300
INFERENCE_FAILURE_THRESHOLD=3
CAPACITY_ACTIVE_LEASE_SECONDS=300
CAPACITY_COOLDOWN_SECONDS="${LOCAL_APPRAISAL_CAPACITY_COOLDOWN_SECONDS:-120}"
WATCHDOG_LOG_FILE="${LOCAL_APPRAISAL_WATCHDOG_LOG_FILE:-$HOME/Projects/Girl-Agent/logs/local-appraisal-watchdog.log}"
HEALTH_PAYLOAD="{\"model\":\"${HEALTH_MODEL}\",\"messages\":[{\"role\":\"user\",\"content\":\"只输出0\"}],\"max_completion_tokens\":1,\"temperature\":0}"
CAPACITY_TOKEN=""

log_watchdog() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') local-appraisal $1" >> \
        "$WATCHDOG_LOG_FILE"
}
