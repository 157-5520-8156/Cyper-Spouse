...
| Phase 1 | QQ/NapCat 文本和 simulator 经过 `CompanionTurn.respond`；平台、tool、media 与 timeout 可经 `settle` 幂等结算；后台媒体生成、投递和重启恢复也通过同一 `settle` 路径 | HTTP 调试入口与旧式调度器恢复分支仍需完全收口到同一 seam |
...
| Phase 6 | 多段文本 receipt、用户插话取消、QQ 图片/贴纸/反应、后台图片和正常 afterthought 的真实回执语义已覆盖；无 durable receipt 记为 `unknown` | 重启后由调度器补发的文本/afterthought 仍有直发旁路；需补 NapCat 适配器级回归矩阵 |
