...
- World 生产入口已 fail-closed：只要运行时携带 World identity，旧的直接
  `send → confirm_*` 链不会作为回退执行；正常、定时、主动、生活分享和 QQ 后续表达都由
  `CompanionTurn` 负责投递与回执结算。
- QQ 后续气泡在 World mode 仅读取 World 的情绪/关系投影；旧 mood 表仅服务非 World
  兼容模式，不能再把历史残留状态带回当前世界。
- World 对话体验回归已迁到公开的 `CompanionTurn + CaptureTurnTransport + receipt` seam。
  它覆盖空模型输出的一次调用安全回退、未知细节的认识论表达、第一人称用户记忆的归属改写、
  以及分段消息的中断结算；与回合、QQ/NapCat、旧链隔离、hard guard 和延迟评估合跑为
  245 passed（62.59s）。
...
- 真实 QQ/NapCat 样本的统一审计命令：
  ...
  selected affordance 分布、用户情绪/私密印象入账率、`evidence_status`、`experience_status`
  和隐私声明；
...
- `var/evaluation/baseline-live-20260713.json`：真实模型 3 次独立重复；每 variant 24
  个 hot 样本。full hot P50=2449ms、P95=3297ms，满足当前延迟 SLO；该轮有 2 条
  “共同迟到”结果假设的 hard issue，随后已由 Guard 修复。
- `var/evaluation/baseline-live-post-outcome-guard.json`：真实模型抽样复测未再出现该
  hard issue，但只有每 variant 8 个 hot 样本，不能替代前述 latency 样本量门槛，也不
  能替代人工盲评。
