...
### 4.1 Event/checkpoint prefix MMR
...
每个 atomic commit 的所有 event leaf 之后，再 append 一个 checkpoint leaf：

```text
PrefixCheckpointLeafV1 = canonical({
  contract: "world-v2-prefix-checkpoint-leaf.1",
  world_id, commit_id, first_ledger_sequence, last_ledger_sequence,
  cursor, request_hash, result_hash, ordered_event_ids_hash,
  locator_root_after
})
```

它不是业务 event，也不改变 ledger sequence；它是认证索引的一部分。MMR 因而绑定
event 的完整位置、commit 原子边界和每个 historical cursor 的 locator root，而不是
仅绑定 `event_json`。一个历史 cursor 的 checkpoint leaf inclusion 可在最新 anchor
下直接认证其 locator root，无需沿 `previous_checkpoint_hash` 线性回放。event
membership proof 仍验证 leaf 到同一 anchor root；append consistency 只允许先前 root
成为新 prefix 的前缀。
...
每个 commit tail 都生成以下 derived checkpoint record，并将其 digest 作为上一节的
checkpoint leaf 写入 MMR：

```text
PrefixCheckpointV1 {
  contract, proof_version, world_id, cursor, event_count,
  mmr_peaks, event_mmr_root, mmr_leaf_count, locator_root,
  commit_id, commit_record_hash,
  checkpoint_hash
}
```

`commit_record_hash` 包含 commit id、request hash、exact `CommitResult` 和有序 event
ids。checkpoint leaf 使 commit atomicity、event ordering 与 post-commit locator root
一并进入 append-only proof；mid-commit cursor、cross-world cursor、future cursor 或
root/handle 混用都必须拒绝。
...
4. **D：外部 anchor（独立选择）** — 以 `HistoryAnchorPort` 在数据库外持久化单调
   `(world, protocol, latest checkpoint cursor, MMR root, MMR leaf count, head state
   hash)`；仅在 operator 提供 key/存储条件后，开放快速 cold start 与
   anti-tamper/anti-rollback 声称。
