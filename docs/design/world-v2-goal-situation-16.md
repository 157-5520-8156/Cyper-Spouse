...
Goal 也没有 settlement write lane：已提交 occurrence/Fact/Experience 只能作为
`CommittedEvidenceBasis`，由下一次已接受 Deliberation 作 progress、blocker
解释或 strict completion recognition；不能直接提交 `V2Goal*`。共享
`CommittedEvidenceSource(source_kind=character_core)` 的 wire shape 同样不代表
Goal capability：`.16.0` 没有把慢变量人格修订解析为当前 Goal outcome 的 typed
parser，因此该 source 对 Goal 一律以
`character_core is not an installed Goal deliberative source` fail closed。未来若要
启用，必须新增独立的、actor/field-class/revision/privacy/capability 全部 pinned 的
Goal parser 与 replay/attack fixtures，不能复用 opaque Core ref。
