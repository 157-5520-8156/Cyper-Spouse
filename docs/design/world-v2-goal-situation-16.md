...
| Resource adjust/reclassify | operator；typed deliberative basis/internal self-assessment | settlement adapter、random draw |
...
| Resource | `V2ResourceStateAdjusted` | adjust/reclassify | deliberative/operator | exact typed cause + before/delta/after + pinned band policy；settlement adapter未安装 | resource head/history |
...
| GS16-RES-010 | state_change delta=0，或reclassify偷改value/未真实换policy | 拒绝 |
...
| GS16-ATT-009 | 裸focus_ref、错current projection hash/status/actor/world | typed focus resolver拒绝 |
...
- `advance()`在`ClockAdvanced`后确定性枚举due Goal与Attention expiry：Goal写exact mechanical after，Attention只为每个current revision打开一次due trigger；Resource recovery registry为空，必须fail closed且不产event；
