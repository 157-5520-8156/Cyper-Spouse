...
| Event payload catalog | `event_catalog.py::_PAYLOAD_MODELS` 与 domain payload map imports | 四域 payload maps；Attention trigger复用现有 TriggerProcess model | reducer注册了但 catalog拒绝，或 payload只按宽松模型校验 |
...
- [ ] `LocationChangedPayload` 支持 establish/change；establish只允许ActorAuthority operator lane。
- [ ] 非initial Location change只允许active ActorAuthority operator correction或prior-revision exact settled movement；没有generic deliberative lane。
...
- [ ] 新建 `location_reducers.py`：
  ...
  - [ ] ordinary settled movement可改变scene visibility；
  - [ ] privacy_class满足来源floor且普通revision不可无authority弱化；
  - [ ] 用户陈述、模型deliberation、图片背景、未settled external result不能移动角色；
  - [ ] operator初始化/纠错解析active ActorAuthority；
  - [ ] compensation及chronology；
  - [ ] zero-cascade。
...
- [ ] operator establish → settled movement change → exact compensation。
- [ ] 同actor第二个rev1、wrong before、wrong from/to、cross-world source拒绝。
- [ ] 用户说“你在学校”、generic deliberative change、图片背景、pending external result拒绝。
- [ ] scene private→public合法移动可接受；同一mutation偷降privacy_class拒绝。
...
- [ ] compensation exact target event/hash/revision与effective lane攻击。
...
- [ ] pure band/recovery policy函数可被reducer和测试共同调用，但不暴露为caller写接口。
...
- [ ] 新建 `resource_reducers.py`：
  - [ ] 每 `(actor, kind)` 单head；
  - [ ] before+delta=after，范围外拒绝，禁止clamp；
  - [ ] band由payload pinned policy复算；
  - [ ] reclassify delta=0但policy/band必须有实际变化；
  - [ ] Clock recovery只认共享resolver返回的latest Clock projection，并验证exact interval/input digest/policy/after；
  - [ ] duplicate Clock target拒绝；
  ...
  - [ ] compensation exact target event binding/effective lane、chronology、zero-cascade。
...
- [ ] operator initialize三种resource；普通settled adjustment；reclassify；compensate。
- [ ] unknown kind、Budget/need混入、delta不守恒、越界/clamp、semantic no-op拒绝。
- [ ] wrong/unknown band policy、caller伪造band、当前policy静默重算攻击拒绝。
- [ ] exact latest Clock projection recovery；旧projection/cause自报/wrong interval/source digest/policy/after、duplicate Clock拒绝；无需Acceptance。
- [ ] typed Acceptance/hash/CAS/ActorAuthority/selection_mode（random_draw始终fail closed）/typed basis privacy floor/chronology攻击。
- [ ] compensation exact event binding/effective lane攻击。
- [ ] zero-cascade + project/rebuild。
...
- [ ] 新建 `attention_reducers.py`：
  ...
  - [ ] mode/focus/allocation/interruptibility/expires_at不变量；
  - [ ] change/compensation exact event binding/effective lane/CAS/chronology；
  - [ ] DND/available不映射回复行为；
  - [ ] zero-cascade。
- [ ] `schemas.py::TriggerProcess`：
  - [ ] 增加`v2_attention_expiry_due` literal；
  - [ ] 增加typed `AttentionExpiryDueAuthority` 可选字段（不要自由JSON）；
...
- [ ] `reducers.py`：
  ...
  - [ ] `_trigger_process_opened` exact解析Attention head与共享resolver返回的latest Clock projection、expires_at、deterministic trigger ID；
  ...
  - [ ] 同current Attention revision只允许一个active due trigger；stale trigger保留审计但不生效。
...
- [ ] wrong Clock/hash/revision/Attention before/expires_at、重复trigger、未到期trigger拒绝。
...
- [ ] `world_v2/runtime.py` 或现有advance seam枚举due goals/resources/attention，不在Adapter实现领域规则。
- [ ] `V2GoalExpired` event identity由world/goal/before/latest Clock/policy确定。
- [ ] `V2ResourceClockAdjusted` event identity由world/resource/before/latest Clock/policy确定。
- [ ] Attention trigger ID由world/attention revision/latest Clock/policy确定。
- [ ] recovery重用相同event/trigger identity；重复执行严格幂等或明确拒绝，不产生第二个结果。
...
- [ ] ActivityCompleted先commit；Resource pair后，Deliberation对Goal选择no-change（零Goal event）或带class/rationale的positive progress pair，再处理Attention；Situation每步只看已提交head。
...
- [ ] ClockAdvanced产生Goal/Resource mechanical结果和Attention due trigger；recovery不重复。
