...
- [ ] 冻结`.16.0` installed capability matrix：Location operator-only；Resource initialize=operator、state_change adjust=operator/deliberative；`reclassify`仅保留wire shape但registry为空、所有提交fail closed；Attention establish=operator、change=operator/deliberative；三域settlement写adapter全部fail closed。
...
- [ ] `.16.0`只安装operator initialize与operator/deliberative state_change adjust；reclassify、settlement adapter与Clock recovery registry均为空且fail closed。
...
- [ ] 新建 `resource_reducers.py`：
  ...
  - [ ] reclassify wire要求delta=0、value不变且policy artifact必须有实际变化；`.16.0`没有第二个band policy artifact，故registry为空并在mutation前fail closed；
...
- [ ] operator initialize三种resource；operator/deliberative state_change adjustment；reclassify wire/registry-empty拒绝；compensate。
...
当前实现边界：Attention typed mutation已接共享catalog/reducer/SQLite/semantic；expiry纯层wire validator保留，但共享`TriggerProcess v2.1`缺actor与immutable expiry authority，故`.16.0`不安装`v2_attention_expiry_due`，以下expiry shared lifecycle条目留给新bundle，不能视为已完成接线。
...
- [ ] 新建 `attention_reducers.py`：
  ...
  - [ ] typed `AttentionFocusBinding=Plan|WorldOccurrence|Trigger`保留closed wire；`.16.0`只安装Plan/WorldOccurrence exact current resolver，Trigger因共享projection无actor authority而fail closed；focus_ref由binding派生，禁止裸ref；
...
- [ ] typed Plan/Occurrence focus正常；任意Trigger focus在`.16.0`以authority未安装拒绝；裸focus、wrong current status/revision/projection hash/actor/world拒绝。
