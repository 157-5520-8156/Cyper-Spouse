...
> **结构性历史设计。** 本文的生命周期去重思想仍可参考，但其 quick-reaction
> `act/hold`、固定语义失败策略及任何把情绪映射为表达的内容已由 ADR0010/ADR0014
> 明确取代，不能作为当前生产行为规范。旧事件和 reducer 仅为冷重放保留；当前表达由
> `Current Self/Recall → PrivateTurnState → ExpressionDraft` 的角色模型因果链决定。
...
| afterthought（已退役，仅历史重放） | 不再产生新 trigger | 历史 trigger 身份保持不变 | 仅历史 A | 不再抽签 | 不再调用独立模型 | 新表达改由普通 `ExpressionPlan` 或事件驱动主动联系产生 | 无生产受理缝 | schema/reducer 保留旧事件冷重放 | 历史版本冻结，不再演进 |
