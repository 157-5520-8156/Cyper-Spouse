...
状态：内部接入闭环；本文件同时保留需要真实 provider/授权的部署边界
日期：2026-07-16  
读者：World v2 agent、图片机维护者  
相关规格：[World v2 重构计划](../world-v2-refactor-plan.md)、[图片机合同](../media-machine.md)

> 实施状态更新（2026-07-16）：P0 的 source-bound life-share candidate-to-plan
> prefix、P1 的候选选择/接受批次，以及 P2 的普通角色公开 candidate-to-plan
> prefix 已接入 World v2
> 生产组装。`MediaPreviewConductor` 将已打开候选的 bounded selection、Acceptance
> 与 durable planning 收为一个 scheduler seam；它只在 selector、grant/budget 和
> planner 同时显式注入时启用；HTTP/QQ scheduler 会调用它，但默认部署未提供该对象，
> 因而不会因此自动发图。Proposal 后与 Acceptance 后的 SQLite restart 均优先恢复已有
> durable work，不重复 selector 或跳到新候选。P2 的角色合同、AppearanceState 和可选短时身体状态会在
...
> `alluring_life` preview opportunity / planning prefix，支持 `embodied_state` 与已声明的
...
> snapshot`，以及 P3 的
> `生产关系累积→close_friend→私密活动→RecipientScopedImageEvidenceDeclared→
> private 角色候选→opaque token selection→Acceptance→V3 snapshot`；图片桥接
> 测试还覆盖 P2 capture 越权拒绝与 P3 recipient/basis 绑定。
> 真正的 preview 专指 render → inspection → `MediaPreviewGenerated`。production
> 已接入 `plan_to_render` 与 `render_to_inspect` continuation consumer：每一步均经过
> source-bound Proposal、Acceptance、预算、Action、provider result 与重启/并发 join。
> 未显式注入 durable provider/grant/operator approval 的部署仍 fail closed，因此内部
> continuation 完成不等于默认环境已经生成或自动发送真实图片。

## 结论

World v2 已具备完整的内部媒体闭环：不可变 sidecar、候选/机会/计划/验收、render/inspection
continuation、preview、effect-once Action、预算、一次 repair、批准/投递与后续互动线程。公开
life-share、角色公开媒体、有限私密媒体与 object/food 已有 source-bound 输入适配。尚需外部
条件的是其余图片机 v5 family、真实 durable provider、operator grant/approval 和目标 transport
部署，不是 World v2 continuation 机制。
...
| preview、人工批准与 delivery | 已有 `MediaPreview`、approval、`MediaDeliveryShared`、delivery-trigger 与 production continuation | 已闭环；缺部署 provider/grant 时明确 unavailable，不应由图片机自行放宽。 |
| life 事件 → 图片候选 | 已有 `EventEcologyMediaCandidateRuntime` | 仅是第一期窄入口：只处理 `public/shareable`、只产出 `life_share`；production profile 只开 candidate，旧直冻机会仅保留显式兼容开关。 |
| 世界选择“是否拍、拍哪一张、life/character family” | 部分完成 | `MediaSelection`/authorizer 已覆盖已安装的公开 life 与角色 candidate；不代表所有 family 或主动生成均开放。 |
| World v2 → 图片机 v5 planner bridge | 部分完成 | public seam 的 selection/冻结路径已接入；真实 durable provider 和完整 v5 planner 覆盖仍需分别验收。 |
| 事件时刻的可见外观、具体衣装事实 | 部分完成 | 稀疏 Appearance State 已可冻结；仍不等于完整衣柜或任意衣装推断。 |
| 可见身体状态、私密资格事实、受众上下文 | 部分完成 | 限定的 recipient-scoped private candidate-to-plan prefix 已实现；真正 preview 与其他私密等级仍未关闭。 |
...
#### 已实施的 reviewed visual-evidence author（2026-07-20）

P0 关闭后供给侧仍然饥饿：默认 host 没有任何受信 declaration writer，生产账本因此没有
任何 `PhotoCandidateOpened`。现在 life ecology 增加了一个受信、有界的
`LifeVisualEvidenceAuthor` followup（`life_visual_evidence_author.py`）：

- 只消费**已结算**的 `WorldOccurrenceSettled`（aftermath lane 的产物），且该 occurrence
  对应的 reviewed opening 必须在 `world_seed.yaml`（reviewed-life.12 起）注册了
  `visual_evidence` 注册块；注册块的值被逐字复制进声明，从不让模型补写画面。
- “这一刻要不要留作可拍素材”是每个 occurrence 一次的 recorded uniform draw
  （40 个 bucket，seed 绑定 settlement ref，重启/重放稳定），与由 accepted Affect
  调制的确定性阈值比较；低落时压住的同一张票，心情转暖的后续 wake 可以再次越线。
  每日上限（默认 3）与最小间隔（默认 2h）在 draw 之前生效。
- 公开/可分享 occurrence 走 `ImageEvidenceDeclared`（含可选 `character_media`
  capture 事实，为 selfie/check-in 候选供证）；`private_transition` 开口在关系阶段
  ≥ close_friend 且配置了 counterpart 时走 `RecipientScopedImageEvidenceDeclared`
  （P3 私密 selfie/mirror 供给）。声明落账后同一 pass 内让
  `CharacterMediaCandidateRuntime.open_once` 开出 fact-bound 候选，media ecology
  在其后的 drain 中开出 life_share 候选。
- 它不开 opportunity、不规划、不渲染、不投递；selection/acceptance/planning 仍
  由显式注入的 `MediaPreviewDeployment` 与 grant/budget 决定，默认部署继续 fail closed。

#### 已实施的 object / food authority vertical（2026-07-16）

`FactCommittedV2.value_ref/value_hash` 仍然**不是**视觉描述，不能被编译器、生态或图片模型
解释成食物、饮料或物品。为此补入了窄的 `VisualFactRecorded` vertical：可信调用方只能指定一个
已提交生活事件和完整的 `VisualFactContentV1`；运行时先把 exact JSON 写入 immutable media
sidecar，再把 source event ref/hash、facet、privacy、content ref/hash 写入账本。账本描述符不重复
`description`，所以丢失或重绑 sidecar 时只能 `NotRenderable`，不会退回成“泛化美食图”。

当前开放 facet 仅为 `activity.visible_object`、`meal.visible_food` 与 `meal.visible_drink`。生态把
`VisualFactRecorded` 当作其自身的 accepted visual declaration；`MediaEvidenceSnapshotCompiler` 只能
在精确 hash、content type、schema、facet/subject/visibility 全部匹配后读取 sidecar，并为每个对象
leaf 绑定该 descriptor event 的 provenance。该 lane 已接入 production composition 的 ecology 和
selection compiler，但默认 host 尚未自行产生受信 VisualFact 命令；这仍是上游生活/工具/运营观察
writer 的后续接入工作。

### P1：候选选择取代 ecology 直冻机会

- [x] `PhotoCandidate` 完整生命周期：`available → selected → planned/generated/shared` 与 `skipped/unrenderable/expired/failed`。
- [x] `MediaSelection` Proposal + Acceptance + `MediaOpportunityAuthorizer`。
- [x] 将当前 ecology 的“直接机会”移到兼容开关；新路径只开 candidate。
