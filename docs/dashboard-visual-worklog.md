...
- 整屋草稿视觉检查通过：边桌在餐区右侧，脚凳在前景地毯边缘，落地灯未侵入屋顶或墙面。
- 新增通用 Runtime 删除回归：对全部 63 个 art-draft 对象逐一执行 hidden/solo，验证 attachment 子树只随其真实父对象隐藏、solo 只保留祖先链，并针对全部已声明 interaction approach 锁定寻路结果。逐像素浏览器视觉基线仍需在最终验收阶段与每次重生成一并复查。
