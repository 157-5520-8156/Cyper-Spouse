...
当前还加入了轻量日程线：上午自习/课间、午饭买饮料、下午课或自习、晚上收尾、睡前刷手机。它不追求复杂日历，而是保证主动消息、自拍和生活事件来自同一个连续日常。
...
- 关键关系事件会比普通消息权重更高：记得她的小事、认真修复、解释冷场都会写入 `key_relationship_event`，并给关系推进额外但有限的加权。
...
- 普通回复 prompt 会读取最近她自己问过几次问题，形成追问预算。最近问得多时，下一轮优先陈述、接话或轻微分享，不继续好奇宝宝式追问。
- QQ 私聊入口有保守回复时机判断：纯确认词可以不回且不标未读；问题、情绪和“在吗”立即回；长篇叙事在忙碌时段可以延迟，并在稍后回复时带入“刚看到”的上下文。

## EchoText Ideas Ported

## Life Projection Rule

生活运行时是所有长期行为状态的共同落点，而不是又一套孤立状态机。

- 任意会改变 `MoodState`、关系印象、未完成对话、主动反馈或边界的业务路径，保存状态后必须调用 `synchronize_life_runtime(...)`。
- 生活运行时将活动本身的基础注意力、短期用户事件余波和慢性状态影响合成为当前手机注意力与主动可用性；不要在新业务里直接堆叠修改 `attention_demand`。
- 用户事件可以产生有过期时间的生活余波；情绪、关系、印象等持续状态产生慢性轨迹影响。两者均须能自然消退。
- 新增机制至少要写一条测试，证明它会影响回复时机、主动消息资格、生活事件选择或当前生活上下文中的一项；若它不该影响生活，必须在实现处说明原因。
- 投递失败不得被写成已发生的共同事件；成功发送后才允许形成等待反馈、共享记忆或生活后果。
...
- prompt 中注入行为指导，而不是让模型直接照读数值。主回复上下文也会携带 Plutchik 主导情绪的行为指导（`state_effect_summary` 读取 `emotion_snapshot`），主动消息则继续使用完整的 `emotion_context_line`；两者都不泄漏内部数值。
- MBTI 气质锚点（`mbti_temperament_note`）随自我核心块进入回复 prompt，让 INFJ 等气质在表达层可感知，而不只是影响初始情绪基线。
- 启动时只初始化一次初始状态，避免重启 daemon 后把长期心情和关系状态覆盖掉。
- 角色 reaction selection：根据用户消息造成的情绪 delta 推断 heart/haha/wow/sad/fire/like/star/bolt 等轻反应。除影响贴纸意图外，NapCat/OneBot 通道会通过 `set_msg_emoji_like` 把它变成真实的 QQ 表情回应，先贴表情、后打字回复，行为像真人先“回应”再说话。
- 未接入主链路的 ST context emotion bleed 实验模块已移除，避免留下看似可用但没有真实业务入口的状态路径；未来若接入外部上下文，应以明确的事件适配器重新实现并加端到端测试。
- 情绪化回复时机模型：温暖状态更快读/回，生气、低落、不安时读回更慢。高波动时 `reply_decision` 可走真正的 read-but-not-reply（`emotional_ghost` → `reply_later`，`mark_unread=False`），稍后补回时带“当时故意没马上回”的上下文；短停顿仍由 `emotion_reply_timing.ghost_delay_ms` 拉长打字前等待。见 `tests/test_reply_decision.py` 与 `tests/test_qq_websocket.py::test_upset_state_ghosts_through_read_later_and_uses_ghost_context_hint`。
