...
- ST context emotion bleed 的思想：外部上下文只做低权重情绪渗透，并设置单项/总量上限，避免状态被打爆。当前这是可用模块，尚未接入 QQ 主链路；只有未来 SillyTavern/MCP 外部上下文进入核心状态时才应启用。
- 情绪化回复时机模型：温暖状态更快读/回，生气、低落、不安时读回更慢，必要时出现短 ghost delay；QQ 发送层会把它和基础读字/打字时间合并，而不是完全替代。
