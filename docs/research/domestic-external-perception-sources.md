...
路由连通、解析、稳定 ID、重复抑制及 observation-time → live snapshot → 冷重放验证。

项目所有者在 2026-08-03 明确要求开放自动抓取，但这项授权只能代表 Girl-Agent 的部署
意图，不能代替第三方平台或聚合服务授予数据获取、缓存和模型处理权。知乎和 B 站的现行
协议已在上文明确禁止未经许可的自动化收集/爬取，其他路线也没有可审计的上游许可。因此
当前 10 个 source 仍统一标记为 `disabled-unlicensed`，生产不会抓取、缓存、暴露给角色或
冻结到 World；不能用 `policy_owner_ref` 把缺失的第三方许可伪装成已有授权。

技术路径已经支持无发布时间的趋势观察：保留 `published_at = null`，并使用独立的
`observed_at` 进入证据窗和可重放快照，不能拿观察时间冒充发布时间。未来取得官方 API、
书面许可或许可范围明确的商业聚合接口后，应创建新的 source policy revision，并确保
raw evidence 保留时间严格长于 attention merge window，再开放生产抓取。
