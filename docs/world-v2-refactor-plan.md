...
|---|---|---|---|
| Fact 完成态 | 在 `.12` 完成 authority；当前 `.13` 继续携带 | **首次完成 `.12`；当前代码常量 `.13`** | 仍需随全量 Runtime 集成验证 Capsule/retrieval 消费 |
| Experience A2 | `.13` 工作树实现中，**尚未完成** | **当前代码常量 `world-v2-reducers.13`** | proposal 对未来 settlement evidence 的时序矛盾、occurrence 同 settlement UoW 正例、SQLite `.12→.13`、source/identity/privacy/time/replay 攻击、全量 tests/ruff 尚未关闭 |
| CharacterCore | 冻结设计；当前 schema 仅旧占位 | 计划 `.14` | typed initialize/revise/compensate、field lane、evidence window、migration 未实现 |
| Goal + Location/Resource/Attention authority | 冻结设计；旧 Goal 和裸字符串 Situation 不合格 | 计划 `.15` | typed heads、Clock mechanics、zero-cascade、legacy quarantine 未实现 |
| SituationCompiler 完整 slice | 冻结设计；当前 `current_situation` 是 unavailable slice | 与 `.15` authority 同阶段开放，Compiler 本身不成为写 reducer | source matrix、semantic hash、redaction、Capsule 消费未实现 |
| MemoryCandidate F2 | 冻结设计 | 计划 `.16` | Reinforced/Forgotten/Compensated 显式事件、source-bound retrieval、migration 未实现 |
...
`.13` 常量已经写入工作树，但这只表示正在施工的目标 bundle，不代表 Experience A2 已验收。未来编号是当前最佳串行合并顺序，不是已发布声明。若其他已冻结 authority 先合并，后续编号整体顺延；每次合并必须更新本表、代码常量、event catalog、semantic payload 和 SQLite migration，禁止两个不同语义共用一个 bundle 号。
...
3. **2C Experience A2 `.13`（当前施工中，未验收）**：先解决 proposal 在 occurrence settlement 前无法引用“未来 evidence”的时序矛盾，再完成 immutable `experience.1`、occurrence/receipt source binding、同 settlement UoW、legacy-unverified 隔离、proposal adjacency、SQLite `.12→.13` 非空 migration 和 source/identity/privacy/time/zero-cascade/replay 攻击测试。Experience 是 CharacterCore/Memory 的长期 evidence 基座，因此先于二者。
4. **2D CharacterCore `.14`**：实现 initialize/revise/compensate、immutable/operator/slow field lanes、跨场景 evidence window 和 privacy projection。它先于完整 Situation/Capsule，避免 Compiler 继续读取无 authority 的 traits 字符串。
5. **2E Goal + Situation constituents `.15`**：一次关闭 Goal、Location、Resource、Attention 四个独立 authority Module，以及 Clock due/expiry、compensation、zero-cascade 和 legacy Goal quarantine。SituationCompiler 仍是只读 Module，不注册 `SituationChanged` reducer。
6. **2F MemoryCandidate F2 `.16`**：在 Fact/Experience/Thread source authority 稳定后实现 candidate lifecycle，尤其显式 `Reinforced/Forgotten`、privacy ceiling 与 source-bound retrieval。
7. **2G ExpressionPlan/Beat `.17`**：在 Action/receipt authority 已稳定后实现 universal one-beat/multi-beat path、用户插话 reconsideration、Logical Clock delay、Plan terminal settlement，并删除自由文本旁路。
8. **2H 其余 vertical 收口**：Affect、relationship、NPC、Thread、Commitment、Grant 等已存在 Module 继续按各自 authority tests 修复；任何语义变化单独升 bundle，不塞进上述编号。
...
Phase 0
  └─ Phase 1 schemas/interfaces
       ...
       │    └─ Experience A2 .13
       │         ├─ CharacterCore .14
       │         │    └─ Goal/Situation constituents .15 ─ Phase 3 capsule/catalog
       │         └─ MemoryCandidate F2 .16 ────────────────┘
...
| `.14` | CharacterCore | 旧裸 core 字符串只作 import candidate/operator review，不自动成为 slow-field authority |
| `.15` | Goal/Location/Resource/Attention | 旧 Goal/needs/location/attention 归档或 unavailable；不保留旧隐式 completed 语义 |
| `.16` | MemoryCandidate F2 | 旧 memory row 只生成 pending import candidate，必须重新绑定 Fact/Experience/Thread source |
...
1. 当前工作树代码常量已经是 `world-v2-reducers.13`，但 Experience A2 尚未验收。先解决 proposal 与未来 occurrence settlement evidence 的时序矛盾，补 occurrence 同 UoW、SQLite `.12→.13`、source/identity/privacy/time/zero-cascade/replay 攻击测试，再跑全量 tests + ruff；完成前不启动 `.14` 合并。
2. 按 Phase 2C–2G 的顺序一次只合并一个 authority vertical；可以并行准备 contract/attack fixtures，但 bundle/migration 必须串行落地，防止共享 schema/reducer version 冲突。
3. `.13` 后实现 CharacterCore `.14`，随后 Goal/Location/Resource/Attention `.15`；只有 `.15` authority 与 source matrix 通过后，才把 `current_situation` 从 unavailable 改为可用。
4. MemoryCandidate `.16` 必须包含 `Reinforced/Forgotten` 显式事件和旧 memory quarantine；Expression `.17` 必须让普通单段回复也走 one-beat plan，并删除自由文本 Action 旁路。
5. 每包执行 11.2.1 的 proposal、source、clock、privacy、zero-cascade、SQLite/replay/tamper 攻击套件，并更新 10.0 的“实际 bundle”，不能只改计划号。
6. 完成 authority 竖切后再把它接入 SituationCompiler/Deliberation；接入 trace 必须证明“来源 → head → Capsule → 主模型选择 → Action/无 Action → settlement → 后续消费”，但不得给矩阵添加固定话术映射。
7. 继续暂停旧 Engine 上的非 P0 行为补丁；平台和 QQ 仍最后迁移，本轮 authority 测试不依赖 QQ。
