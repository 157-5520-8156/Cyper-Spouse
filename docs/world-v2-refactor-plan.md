...
|---|---|---|---|
| Fact 完成态 | **已在 `.12` 完成、验收并提交（`62ab8d6`）**；后续 bundle 保持同一语义继续携带 | **首次完成 `.12`；当前代码常量 `.14`** | 仍需随 Runtime 集成验证 Capsule/retrieval 的实际消费；这不是 Fact authority 未完成 |
| Experience A2 | **已在 `.13` 完成、验收并提交（`ce0d9f3`）**：immutable `experience.1`、typed proposal/Acceptance、occurrence/receipt exact binding、legacy quarantine、SQLite migration 与攻击测试均已关闭 | **已完成 `.13`；由 `.14` 继续携带** | 尚未接入 Situation retrieval；不得把“消费者未实现”倒写成 Experience authority 未完成 |
| MemoryCandidate F2 | **已在 `.14` 完成、验收并提交（`1ab8685`）**：全量验收 `394 tests`，审查 `P0=0、P1=0` | **已完成 `.14`** | 尚需随 SituationCompiler/Runtime 集成验证 source-bound retrieval 的实际消费；这不是 MemoryCandidate authority 未完成 |
| CharacterCore | 冻结设计；作为当前下一 authority vertical | 计划 `.15` | typed initialize/revise/compensate、field lane、evidence window、migration 未实现 |
...
`.12` Fact、`.13` Experience 与 `.14` MemoryCandidate 的完成状态以已提交代码、可 replay 的 SQLite migration、authority/attack tests 和全量静态检查为证，不因后续消费者尚未接线而降回“施工中”。MemoryCandidate `.14` 已由提交 `1ab8685` 关闭，并通过 `394 tests` 与 `P0=0、P1=0` 审查；当前下一片为 CharacterCore `.15`，后续是 Goal/Location/Resource/Attention `.16`，SituationCompiler 随 `.16` authority 开放但不虚构写 reducer，Expression `.17`。未来编号仍是当前最佳串行合并顺序，不是已发布声明；每次合并必须同步代码常量、event catalog、semantic payload 和 SQLite migration，禁止两个不同语义共用一个 bundle 号。
...
5. **2E CharacterCore `.15`（当前下一片）**：实现 initialize/revise/compensate、immutable/operator/slow field lanes、跨场景 evidence window 和 privacy projection。它先于完整 Situation/Capsule，避免 Compiler 继续读取无 authority 的 traits 字符串。
...
1. Fact `.12`、Experience A2 `.13` 与 MemoryCandidate F2 `.14` 已完成并验收；MemoryCandidate 提交为 `1ab8685`，全量验收 `394 tests`，审查 `P0=0、P1=0`，当前代码常量为 `world-v2-reducers.14`。不得重复施工这三个 authority，也不得因 Situation/Runtime 消费尚未实现而把它们标回未完成。
2. 当前下一片实现 CharacterCore `.15`：关闭 typed initialize/revise/compensate、immutable/operator/slow field lanes、跨场景 evidence window、privacy projection、legacy quarantine、SQLite migration 与 replay/attack tests。
...
4. CharacterCore `.15` 完成后实现 Goal/Location/Resource/Attention `.16`；只有 `.16` constituent authority 与 source matrix 通过后，才把 `current_situation` 从 unavailable 改为可用。SituationCompiler 是只读 Module，不单独占 reducer bundle。
