...
SituationCompiler 不使用上述 viewer-facing summary。正式 Context 读取 Interface 是
`context_capsule_compiler_from_ledger(...).compile(query)`；其实现
`LedgerProjectionContextResolver` 在一个完整 cursor 上读取 reducer projection、解析逐项
authority source binding，再调用 SituationCompiler：

```text
ContextCompileQuery(world_id, actor_ref, trigger_ref, complete cursor)
  -> LedgerProjectionContextResolver.resolve(query)
  -> source-bound ContextCapsuleRequest
  -> ContextCapsuleCompiler.compile(query)
  -> ContextCapsule
```

该 Module 是 Deliberation 唯一正式内部 Context seam。它必须从同一个 reducer head
构造 CharacterCore、Fact、Experience/Memory、Situation、Commitment/Thread、Affect、
Relationship、Capability/Budget 与 PrivateImpression slice；每项都绑定精确事件来源并经过
actor/subject relevance scope 和 privacy floor。authority 缺失、矛盾或不能完整解析时整个
slice 为 `unavailable`；只有 reducer head 存在且来源闭包得到证明后，空集合才表示“权威为空”。
外部 viewer 永远走 `project(ProjectionRequest)` 的裁剪/脱敏结果。

`InternalProjectionReader` 旧兼容 seam 已删除：它的无 actor/trigger 快照无法与正式
relevance/privacy/source-binding 语义等价，并且曾将已经实现的 reducer head 永久误报为
`reducer_not_implemented`。Acceptance 与 recovery 不得消费有界 Context Capsule，而使用
`InternalAuthorityReader` 的 revision-pinned 按 ID 查询和稳定分页（Action、预算、回执等）；
分页必须遍历到 `complete=true` 或 fail closed，不能因 Context 上限漏掉旧 Action。两种用途
读取同一 Reducer projection，不形成第二套写权威。
...
AdvisoryCompiler → read-only source-bound Context slices, classifier Adapters, MatrixCatalog
...
**当前实现状态（2026-07，必须按部署 profile 区分能力）**：主模型的一次
`ExpressionDraft` 同时选择 `timing_choice=now|later|silent` 与有序表达 beat；不存在第二个
“社交裁决模型”，也没有 helper 可以越过这次选择直接发送。NapCat 私聊 profile 已安装
`text`、`reaction`、标准 QQ `sticker` 与 `typing`：draft materializer 只接受 catalog token，
Acceptance 从 verified Observation 反向校验 reaction 的原始 provider message binding，随后每个
beat 独立形成 Action 并由 QQ transport 调用对应 NapCat endpoint。HTTP capture、QQ official 与
...
| `context_assembler.py` | revision-pinned `LedgerProjectionContextResolver` + typed Situation constituents | SituationCompiler → source-bound ContextCapsule | 无 | `W2-SIT-001..005` + Capsule budget/stability | 收入 deep implementation；删除无 actor/trigger 的旧 snapshot reader；无 `SituationChanged` |
...
### 10.0 截至 2026-07-16 的真实实现基线

本文同时包含已实现合同与冻结的未来设计。状态以代码中的 `REDUCER_BUNDLE_VERSION`、可 replay 的 SQLite migration 和通过的 authority tests 为准，不以文档段落或工作树文件存在为准。

| Authority vertical | 当前状态 | 当前权威证据 | 不得虚报的边界 |
|---|---|---|---|
| Ledger kernel + Fact/Experience/Memory/Core | **完成** | reducer bundle `.32`；typed authority、SQLite migration/reopen/rebuild、source/privacy/tamper/zero-cascade fixtures | 真实用户长期记忆质量仍需外部体验数据 |
| Goal/Location/Resource/Attention + SituationCompiler | **完成** | 唯一逻辑时钟、typed heads、source matrix、viewer redaction、bounded Capsule 与 production consumption | Observation 墙钟不推进世界时钟；缺失来源保持 unavailable |
| Appraisal/Affect/Relationship/NPC/Thread/Commitment | **完成生产闭环** | source-bound trigger→Proposal/Acceptance→head→下一轮 Context；世界事件、用户输入、负面情绪、修复和 withdrawal review fixtures | 模型判断可以选择不安慰/不修复；机械闭环不等于判断永远正确 |
| ExpressionPlan/Beat + social/proactive/pulse | **完成生产闭环** | 同一主模型 draft；now/later/silent、多 beat、reaction/sticker/typing、reconsideration、异常终态、预算/回执/重启 | provider accepted 不等于 delivered；真实平台能力按 profile fail closed |
| Media preview + perception | **完成可注入闭环** | plan→render→inspect→preview continuation、逐步授权、repair/recovery；附件→感知→result trigger→下一轮 Context | 未注入真实 provider/grant 时明确 unavailable；图片结果不反写世界事实 |
| Platform/display/archive isolation | **完成内部迁移** | 默认 ASGI/HTTP/v2 Dashboard、兼容 QQ C2C、平台反向依赖 guard；archive app 显式独立 | 群聊、多用户及外部 provider 部署不是世界机内部验收前置 |
| Evaluator/performance/cost | **完成内部 gate；外部 hybrid 待证据** | 70 fixture/103 unique nodes、1837-test suite、120 scenario `.5`、20-turn hot P95≈0.41s、formal blind artifact pipeline | 真人双盲、真实 provider P95 与账单/token 对账必须由外部工件建立 |

当前 `REDUCER_BUNDLE_VERSION` 为 `world-v2-reducers.32`。早期 `.12`–`.18` 的完成记录是
迁移历史，不再代表当前待办；当前代码已把这些 authority 接入同一 production composition。
任何后续语义变化仍须同步 bundle、event catalog、semantic payload、SQLite migration 与本节
证据，禁止两个不同语义共用一个 bundle 号。
...
4. **2D MemoryCandidate F2 `.14`（已完成、已验收）**：已在提交 `1ab8685` 关闭 candidate lifecycle、exact source binding、privacy ceiling、source-bound retrieval、旧 memory quarantine、SQLite migration 与 replay/attack tests；全量验收 `394 tests`，审查 `P0=0、P1=0`。后续 vertical 只消费这份 authority，不重新发明 MemoryCandidate 写路径。
5. **2E CharacterCore `.15`（已完成、已验收）**：提交 `72eae89` 已关闭 initialize/revise/compensate、immutable/operator/slow field lanes、跨场景 evidence window、privacy projection、operator redo 授权继承、legacy quarantine 与 `.14→.15` SQLite migration；全量验收 `411 tests`，审查 `P0=0、P1=0`。
...
- 离线机制 CI 的输入 corpus 与机制基线必须分开版本化。`world-v2-offline-mechanism-baseline.2` 曾记录 source-bound `private_impressions` Context slice 的加入；其冻结 hash 已在后续接受路径变更中陈旧。`world-v2-offline-mechanism-baseline.3` 原本记录 read-only tool 的 enforcement authorization 进入 `Action` canonical payload 后的身份链变化，但其提交的 hash 不是 clean worktree 的可执行 120 场景结果；`.4` 更正了该 fixture identity。当前 `world-v2-offline-mechanism-baseline.5` 固化 WorldStarted/Observation 逻辑时钟权威和 ExpressionPlan 非 delivered 终态：与 `.4` 的保存 manifest 逐字段比较，120 条 output、terminal action states、trigger kinds、room projection、模型调用、验证结果均相同；120 条 replay hash 因 reducer authority 变化而更新，仅 `provider_timeout.01/.02` 的 event types 增加真实 `ExpressionPlanTerminated`。该基线只证明确定性机制变化，不得解释为人味盲测或真人体验提升。
- 建立机制闭环 CI。
- 删除或隔离旧行为入口。
- World v2 切为默认运行时。
...
上述 70 个 ID 的单一机器可读验收合同为
`src/companion_daemon/world_v2/fixture_acceptance_manifest.py`。每项必须显式列出需求、
机制链、生产代码静态可达锚点、权威事件/投影证据、真实 pytest 节点和 internal/hybrid
验收类别；不得从测试名、120 场景语料的 family 名或文档完成状态反推“已覆盖”。
`tests/world_v2/test_fixture_acceptance_manifest.py` 直接从本节抽取冻结 ID，并验证恰好
70 项、无重复/漏项、测试节点真实存在、生产锚点存在。情绪、关系、记忆、NPC、受控
随机、反抗/不采纳、修复和迁移项必须引用机制链测试，而不能只引用矩阵参数或评估器。
静态存在性检查不构成通过证据；Phase 8 必须运行
`python scripts/verify_world_v2_fixture_nodes.py`，由 pytest 实际执行 manifest 中全部去重
节点。该命令任一节点失败即为 fixture gate 失败，不能用函数仍存在或 marker 命中替代。
真实 provider 的 warm/cold P95、账单/token 对账仍标为 hybrid 外部 gate；其内部记账和
trace 合同通过不等于外部 SLO 已通过。`production_reachability=module_only` 同样是明确
未完成状态：模块测试通过不等于 HTTP/QQ/production turn 已可达；verifier 会防止这些项
被静默提升为 production。
...
1. 用真实模型生成 bare/archive/v2 各 120 场景 × 3 seed 输出，并交给两名独立盲评者；在此之前只声称机械闭环完成，不声称人味已被真人证明。
2. 在目标部署采集至少 20 个完整 hot transport 样本、cold 样本和供应商账单/token 工件；离线 P95≈0.41s 只证明本地世界机热路径，不代表网络 SLO。
3. 部署真实媒体/感知 provider 时必须显式 provision grant、stable lookup 与 operator approval；缺任一依赖继续 fail closed，不回退旧图片机。
4. 后续能力扩展继续执行 11.2.1 攻击套件，并证明“来源 → head → Capsule → 主模型选择 → Acceptance/拒绝 → Action/settlement → 后续消费”。
5. 旧 Engine 只保留显式 archive/兼容入口；不得重新成为默认写权威或承接新的行为机制。
...
## 15. 2026-07-17 线上空世界复盘与补全设计

### 15.1 已证实的生产缺口

QQ v2 的平台入口、时钟、ActionPump 和发送回执都在运行，但这不等于世界机在运行。
线上投影只有消息 Observation 与 ClockAdvanced；Plan、WorldOccurrence、Experience、NPC、
Location、Fact、MemoryCandidate、Thread 和 Commitment 均为空。`LifeEcologyRuntime` 是既有
生活事实的消费者和 fan-out，不是生活事实作者；没有上游时只能持续返回 `idle`。同理，
`ProactiveActionRuntime` 能消费已结算事件和到期 Thread/Commitment，却没有“闲来无事”或
“对方暂未回应”的权威机会来源，因此不会主动发话。

另有两条独立接线错误：QQ 将 `conversation:qq:c2c:*` 投递地址当成用户 subject，导致以
`user:geoff` 入账的 Fact、Memory、Relationship 和 Affect 无法进入下一轮 Context；逻辑时钟
使用 UTC，而时段判断曾直接读取 UTC 小时，导致北京时间凌晨被理解为傍晚。

### 15.2 三个新增深 Module

1. `LocalChronologyCompiler.localize(instant) -> LocalChronology`：唯一负责时区、本地日期、
   weekday、daypart 与昼夜节律。其他模块不得直接读取 UTC `.hour`。
2. `LifeAuthorRuntime.advance_once(wake_event_ref) -> LifeAuthorResult`：唯一负责从已提交时钟、
   静态 seed、当前计划/地点/NPC availability、资源、目标和最近生活史编译合法生活 opening，
   记录受控随机并让 LLM 选择 opening/no-op/合理变体，随后只通过 typed acceptance 写入计划、
   地点、事件、经历和可视事实。
3. `SocialInitiativeRuntime.advance_once(wake_event_ref) -> SocialInitiativeResult`：统一仲裁
   `event_share`、`spontaneous_contact`、`response_gap`，生成一个有来源、有窗口、有 cadence 的
   durable opportunity，再复用 ProactiveDraft → ExpressionPlan → Budget → Action → receipt。

```mermaid
flowchart LR
  Clock["Clock authority"] --> Chronology["LocalChronologyCompiler"]
  Chronology --> Life["LifeAuthorRuntime"]
  Seed["Reviewed typed seed"] --> Life
  Life --> Facts["Plan / Location / NPC / Occurrence / Experience"]
  Facts --> Affect["Appraisal and affect fan-out"]
  Facts --> Media["Visual evidence and media opportunity"]
  Facts --> Initiative["SocialInitiativeRuntime"]
  Chat["Accepted expression + optional response expectation"] --> Initiative
  Initiative --> Expression["ExpressionPlan / Action / QQ receipt"]
  Facts --> Context["Scoped Context Capsule"]
  Affect --> Context
  Memory["Fact-backed Memory"] --> Context
  Context --> Reply["Companion deliberation"]
```

### 15.3 生活 opening 正交矩阵

矩阵只定义模型可见的处境坐标和合法 opening，不把坐标映射成固定行为。

| 轴 | 候选空间 |
|---|---|
| 来源 | routine、intentional_goal、social、environmental_opportunity、interruption、aftermath、user_influence(advisory) |
| 生活域 | sleep_wake、hygiene_private、meal_drink、study_class、creative_photo_writing、commute_walk、errand_household、rest_recovery、family_roommate_friend、digital_leisure |
| 生命周期 | plan、start、pause、resume、complete、abandon、occurrence_commit、activate、observe、settle、expire、aftermath |
| 社交结果 | warm、cooperative、neutral、awkward、disagreement、boundary、repair、cancelled、missed |
| 偏离 | delay、change_plan、avoid、impulse、persist、repair |
| 可视潜力 | none、ambient、place、object、food、activity、social、character、private_transition |
| privacy | public、shareable、personal、private、withhold |

硬 eligibility（时空、已注册 NPC、availability、资源、privacy、来源）先过滤；随机只在合法集上
运行。draw 必须持久化候选集 hash、权重向量 hash、policy/sampler version、seed hash 和结果，
同一 `(world,wake,catalog,slot)` 重试不得重抽。LLM 可以拒绝抽中项或 no-op，但不能返回 event ID、
authority hash、未注册 NPC 或未验证地点。

### 15.4 主动发话矩阵

| opportunity | 主要输入 | 允许的自然结果 |
|---|---|---|
| event_share | salience、novelty、user relevance、shareability、关系、情绪、attention、近期 cadence | now、later、silent；分享、轻描淡写、暂不说 |
| spontaneous_contact | 无聊/孤独/在意/好奇冲动、昼夜适配、自身活动、关系距离、互惠、可用真实话题、cadence、recorded draw | 纯联系、换话题、分享真实生活片段、silent |
| response_gap | 原 beat 的显式 response expectation、重要度/情绪电荷、关系、未完 Thread、投递不确定性、当前活动/情绪、再次发话成本 | 轻推、继续自己的事、换话题、表达在意/不爽/担心、later、silent |

问号不等于期待回复。主回复模型只能提议可选 `ResponseExpectation`；Acceptance 必须绑定已接受
ExpressionBeat、due window、hoped response、pressure/importance 和 provider delivery confidence。
用户新输入在 dispatch 前到达时必须关闭或重新考虑机会。`provider_accepted` 不能被表达成
“你看见了却不理我”。陌生关系与夜间一般进入矩阵而不是一票否决；真实性、隐私、effect-once
和滚动频率上限才是硬边界。

### 15.5 图片机覆盖边界

现有 event ecology 仅能从已经存在的 activity process/result、settled outcome、NPC shared
outcome、shared experience、place/environment、object/food 中发现候选；它不创作这些来源。
生产路径还要求 `ImageEvidenceDeclared` 或 `VisualFactRecorded`，测试中手工塞入 projection 的
丰富度不能视为生产覆盖。Life Author 必须先产生真实 lived event，再从 typed visible slice
声明视觉证据；图片 prompt 或生成结果不得反向制造世界事实。

私密生活事件可以存在，但不自动获得私密图片资格。当前只在 recipient、relationship、basis、
private-transition evidence 全部成立时开放 `alluring_life`；`exclusive_private`、
`suggestive_private`、`explicit_reserved` 在 World-v2 未具备完整权威链前继续 fail closed。

### 15.6 新增验收门

1. 默认 QQ composition 跑 32 轮新人对话，覆盖身份、非助手、自报姓名/偏好跨轮召回、失望与冒犯、
   关系边界、多 beat、当前活动、时段合理性、事件分享和沉默后的主动行为。
2. 跑 7 个逻辑日 × 3 个 seed；Plan、Occurrence、NPC、Location、Experience、Visual source 五域非零，
   无时空冲突，重复率受控，restart/replay hash 完全一致。
3. event-driven、spontaneous、response-gap 都必须经过 QQ host/scheduler/fake delivery 公开 seam；
   now/later/silent、到期前用户回复取消、三类同时到期仲裁、后台积压不饥饿、三个 crash 点 effect-once。
4. 每个媒体候选可追到 declaration 与 source hash；私密事件缺任一授权条件时不得升级或投递。
5. diagnostics 暴露 scheduler last success、initiative last status/reason、pending opportunity/action 数、
   world source counts；“进程 running 但 sources=0”必须明确报告 `starved`。
