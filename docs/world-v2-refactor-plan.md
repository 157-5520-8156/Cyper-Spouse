...
**当前实现状态（2026-07，必须显式区分 vocabulary 与可执行能力）**：矩阵保留
`reaction`、`sticker`、`typing`，以便 LLM 的语义判断可以表达“此处一个非文本动作可能合适”；这
不是生产平台已经支持其外发的声明。当前生产聊天闭环仅开放 `reply`、`followup`、
`proactive_message`（`text/plain`）。三种非文本 action 仅处于 `adapter_only`：
`PlatformActionExecutor` 能校验不变 payload 并绑定 provider receipt，但还没有同时具备
LLM draft materializer、平台专用 transport（QQ C2C 目前也只支持文本）和该 transport 的
recovery lookup。它们因此不得出现在 production chat proposal 的可接受 grammar 中；不能用
“模型认为合适”绕过这个事实。待四个闭环一次性补齐时，再把 capability 提升为 production，
而不是在矩阵中加入行为规则或静默放行。
...
| CharacterCore | **已在 `.15` 完成、验收并提交（`72eae89`）**：全量验收 `411 tests`，审查 `P0=0、P1=0` | **已完成 `.15`** | 尚需由 `.16` SituationCompiler 与后续 Deliberation 消费；这不是 CharacterCore authority 未完成 |
...
`.12` Fact、`.13` Experience、`.14` MemoryCandidate 与 `.15` CharacterCore 的完成状态以已提交代码、可 replay 的 SQLite migration、authority/attack tests 和全量静态检查为证，不因后续消费者尚未接线而降回“施工中”。CharacterCore `.15` 已由提交 `72eae89` 关闭，并通过 `411 tests` 与 `P0=0、P1=0` 审查；当前下一片为 Goal/Location/Resource/Attention `.16`，其冻结施工设计见 `docs/design/world-v2-goal-situation-16.md`（提交 `27252ff`）。SituationCompiler 随 `.16` authority 开放但不虚构写 reducer，之后是 Expression `.17`。未来编号仍是当前最佳串行合并顺序，不是已发布声明；每次合并必须同步代码常量、event catalog、semantic payload 和 SQLite migration，禁止两个不同语义共用一个 bundle 号。
