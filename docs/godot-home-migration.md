...
daemon 的 `dashboard.scene` 是唯一跨运行时的 Interface：

```text
{ location, action, expression, time_of_day }
```

Godot 只能读取它，不能写入 daemon 或世界事实。`GodotRoom` 把 manifest 读取、投影、
导航、遮挡、角色动作和光照隐藏在一个 Module 中，对主场景只暴露：
