...
> **历史修复记录，不是当前行为规范。** 其中本地可见兜底、`act/hold` 和其他旧表达策略
> 已被 ADR0010 与 ADR0014 取代。当前生产路径不得从本文件恢复固定接话、情绪话术或
> 随机语义否决；技术失败走 Expression Reliability Lifecycle，只有角色模型的合法
> Proposal 才能代表 `now / later / silent`。
