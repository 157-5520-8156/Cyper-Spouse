...
- [ ] 波次 3：地毯、灯具、植物与窗区（进行中：四块地毯、两株落地植物、三盏台灯及两件承载柜已进入草稿，现为 40 对象、50 个构建资产；所有屋顶对象排除，其余植物和窗区待处理）。
...
当前进度：四块地毯与两株落地植物已通过草稿检查点。`desk-lamp / bedside-lamp / foreground-table-lamp` 均以 `front + light` 双层对象接入，并分别附着到书桌、床头柜和前景长柜；两件新载体先于灯具独立生成，避免浮空或重复所有权。clean shell 候选移除了烙在墙地面的局部灯斑。餐厨吊灯及所有屋顶对象按用户观测规则排除。当前共 40 个草稿对象、50 个构建资产。
